...
5. Click **Finish** to save. Then, repeat the process to create a mapping for the user's _group memberships_:
    - **Name**: `omada_usergroup_name`
    - **SAML Attribute Name**: `usergroup_name`
    - **Friendly Name**: Leave blank
    - **Expression**:

    ```python
    omada_groups = [g.name for g in user.groups.all() if g.name.startswith("Omada-")]
...
3. Set a name for the group (e.g. `Omada-admins`) and click **Create**. The group name must start with `Omada-` for the `omada_usergroup_name` property mapping to function correctly.
...
9. Set the desired **SAML Role Name**, **Role**, **User Type**, and **Privileges** for the new SAML role. The **SAML Role Name** must exactly match the name of the previously created authentik group. It is case sensitive.
10. Click **Create**.
...
<Tabs
  ...
  values={[
    {label: 'Linux and macOS', value: 'linuxmac'},

...
- **Policies in the application wizard**
    ...
    In the application creation wizard, administrators can now configure policy bindings along with the other application settings.
...
- **OAuth2 provider federation**
    ...
    Configure [OAuth2 provider federation](../../add-secure-apps/providers/oauth2/machine_to_machine.mdx#authentik-issued-jwts) to allow exchanging authentication tokens between multiple providers.

...
:::caution
Note that an `!Enumerate` tag's iterable can never be an `!Index` or `!Value` tag with a depth of `0`. Minimum depth allowed is `1`. This is because a depth of `0` refers to the `!Enumerate` tag the `!Index` or `!Value` tag is in, and an `!Enumerate` tag cannot iterate over itself.
:::

...
class GitlabIssueSyncSpec(IssueSyncIntegration):
    ...
    def split_external_issue_key(
        ...
        Format is "{domain_name}:{path_with_namespace}#{issue_id}"
        """
        try:
            _, project_and_issue_id = external_issue_key.split(":", 1)
            project_id, issue_id = project_and_issue_id.split("#")
        ...
        **kwargs: Any,
        ...
        try:
            ...
            client.update_issue_assignees(
                encoded_project_id, issue_id, [gitlab_user_id] if gitlab_user_id else []
            )
    ...
    def update_comment(self, issue_id, user_id, group_note):
        ...
        project_id, parsed_issue_id = self.split_external_issue_key(issue_id)

        if not project_id or not parsed_issue_id:
            logger.error(
                "update-comment.invalid-key",
                extra={
                    "provider": self.model.provider,
                    "external_issue_key": parsed_issue_id,
                },
            )
            return None
    ...
    def create_comment(self, issue_id, user_id, group_note):
        ...
        return self.get_client().create_comment(
            encoded_project_id, parsed_issue_id, {"body": quoted_comment}
        )

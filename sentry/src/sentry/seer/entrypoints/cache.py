...
class SeerOperatorAutofixCache[CachePayloadT]:
    ...
    def populate_pre_autofix_cache(
        ...
        with SeerOperatorEventLifecycleMetric(
            interaction_type=SeerOperatorInteractionType.OPERATOR_CACHE_POPULATE_PRE_AUTOFIX,
            entrypoint_key=entrypoint_key,
        ).capture() as lifecycle:
            cache_key = cls._get_pre_autofix_cache_key(
                entrypoint_key=entrypoint_key, group_id=group_id
            )
            lifecycle.add_extras({"group_id": group_id, "cache_key": cache_key})
    ...
    def populate_post_autofix_cache(
        ...
        with SeerOperatorEventLifecycleMetric(
            interaction_type=SeerOperatorInteractionType.OPERATOR_CACHE_POPULATE_POST_AUTOFIX,
            entrypoint_key=entrypoint_key,
        ).capture() as lifecycle:
            cache_key = cls._get_post_autofix_cache_key(
    ...
    def get(
        ...
        """
        with SeerOperatorEventLifecycleMetric(
            interaction_type=SeerOperatorInteractionType.OPERATOR_CACHE_GET,
            entrypoint_key=entrypoint_key,
        ).capture() as lifecycle:
            lifecycle.add_extras({"group_id": group_id, "run_id": run_id})
            if not group_id and not run_id:
                lifecycle.record_failure(failure_reason="bad_arguments")
                raise ValueError("At least one of group_id or run_id must be provided.")

            cache_result = None
            if run_id:
                cache_result = cls._get_post_autofix_cache(
                    entrypoint_key=entrypoint_key, run_id=run_id
                )
        ...
        *,
        ...
        for entrypoint_key in autofix_entrypoint_registry.registrations.keys():
            with SeerOperatorEventLifecycleMetric(
                ...
                lifecycle.add_extras({"group_id": from_group_id, "run_id": to_run_id})

...
class PreprodArtifactEndpoint(OrganizationEndpoint):
    ...
    def convert_args(
        ...
        **kwargs: Any,
        ...
        # Use has_project_access (which respects org-level roles like owner/manager)
        # instead of the old get_projects() approach which used has_project_membership
        # (team-only) when no project_ids were specified.
        # Staff are also exempt since subclass endpoints (e.g. download) may use
        # StaffPermission which bypasses the normal org-scoped access setup.
        if (
            not is_rpc_authenticated
            and not is_active_staff(request)
            and not request.access.has_project_access(project)
        ):
            raise HeadPreprodArtifactResourceDoesNotExist

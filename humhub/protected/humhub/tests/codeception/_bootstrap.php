...
defined('YII_TEST_ENTRY_URL') or define('YII_TEST_ENTRY_URL', parse_url((string) Configuration::config()['config']['test_entry_url'], PHP_URL_PATH));
...
$_SERVER['SERVER_NAME'] = parse_url((string) Configuration::config()['config']['test_entry_url'], PHP_URL_HOST);
$_SERVER['SERVER_PORT'] = parse_url((string) Configuration::config()['config']['test_entry_url'], PHP_URL_PORT) ?: '80';

// Set alias
$config = Configuration::config();

$config['test_root'] ??= dirname(__DIR__);
$config['humhub_root'] ??= dirname(__DIR__, 4) . '/';

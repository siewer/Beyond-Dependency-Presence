<?php

return [
    '"My Spaces" Sorting' => '"マイスペース" の並べ替え',
    '1 month' => '1ヶ月',
    '1 week' => '1週間',
    '1 year' => '1年',
    '2 weeks' => '2週間',
    '3 months' => '3ヶ月',
    '6 months' => '6ヶ月',
    '<strong>Confirm</strong> icon deletion' => '<strong>確認</strong> アイコンの削除',
    '<strong>Confirm</strong> image deletion' => '<strong>確認</strong> 画像の削除',
    '<strong>Confirm</strong> topic deletion' => '<strong>確認</strong>トピックの削除',
    '<strong>Mobile App</strong> Debug Page' => '<strong>モバイルアプリ</strong>デバッグページ',
    'Access Token' => 'アクセス Token',
    'Access token is not provided yet.' => 'アクセス Tokenはまだ提供されていません。',
    'Add OEmbed provider' => 'OEmbedプロパイダを追加する',
    'Add Topic' => 'トピックを追加',
    'Add custom info text for maintenance mode. Displayed on the login page.' => 'メンテナンスモードのカスタム情報テキストを追加します。 ログインページに表示されます。',
    'Add individual info text...' => '個別の情報テキストを追加します...',
    'Add new provider' => 'プロパイダを追加する',
    'Advanced Settings' => '高度な設定',
    'Allow Self-Signed Certificates?' => '自己署名証明書を許可する？',
    'Allow establishing verified connections with the mobile app to enable Android app links and iOS universal links and redirect web content to the mobile app.' => 'モバイル アプリとの検証済み接続の確立を許可して、Android アプリ リンクと iOS ユニバーサル リンクを有効にし、Web コンテンツをモバイル アプリにリダイレクトします。',
    'Allowed file extensions' => '許可されたファイル拡張子',
    'Appearance Settings' => '外観の設定',
    'Auto format based on user language - Example: {example}' => 'ユーザー言語に基づく自動フォーマット - 例：{example}',
    'Back to overview' => '概要に戻る',
    'Base URL' => 'ベースURL',
    'Comma separated list. Leave empty to allow all.' => 'コンマ区切りリスト。すべてを許可するには空のままにします。',
    'Configuration (Use settings from configuration file)' => '構成 (構成ファイルの設定を使用)',
    'Convert to global topic' => 'グローバルトピックに変換',
    'Could not send test email.' => 'テストメールを送信できませんでした。',
    'Currently no provider active!' => '現在、有効なプロパイダはありません',
    'Currently there are {count} records in the database dating from {dating}.' => '現在{dating}からデータベースに{count}レコードがあります。',
    'Custom DSN' => 'カスタム DSN',
    'Custom sort order (alphabetical if not defined)' => 'カスタムの並べ替え順序 (定義されていない場合はアルファベット順)',
    'Custom sort order can be defined in the Space advanced settings.' => 'カスタムの並べ替え順序は、スペースの詳細設定で定義できます。',
    'DSN' => 'DSN',
    'DSN Examples:' => 'DSN 例：',
    'Dashboard' => 'ダッシュボード',
    'Date input format' => '日付入力形式',
    'Debug' => 'デバッグ',
    'Default' => 'デフォルト',
    'Default Timezone' => 'デフォルトのタイムゾーン',
    'Default language' => 'デフォルト言語',
    'Default pagination size (Entries per page)' => 'デフォルトのページネーション数（ページあたりのエントリ数）',
    'Default stream content order' => 'デフォルトのストリームコンテンツ順序',
    'Delete' => '削除',
    'Do you really want to delete this topic?' => '本当にこのトピックを削除しますか？',
    'E-Mail' => 'Eメール',
    'E-Mail reply-to' => 'メールの返信先',
    'E-Mail sender address' => 'メールアドレス',
    'E-Mail sender name' => 'メール送信者名',
    'E.g. http://example.com/humhub' => '例）http://example.com/humhub',
    'Edit OEmbed provider' => 'OEmbedプロパイダを編集する',
    'Embedded content requires the user\'s consent to be loaded' => '埋め込みコンテンツを読み込むには、ユーザーの同意が必要です',
    'Enable Link Redirection Service' => 'リンクリダイレクトサービスを有効にする',
    'Enable maintenance mode' => 'メンテナンスモードを有効にする',
    'Enable user friendship system' => 'ユーザーフレンドシップシステムを有効にする',
    'Enabled' => '使用可能',
    'Enabled OEmbed providers' => '有効になっているOEmbedプロパイダ',
    'Endpoint Url' => 'エンドポイントURL',
    'Exclude media files from stream attachment list' => 'ストリーム添付ファイルリストからメディアファイルを除外する',
    'Firstname Lastname (e.g. John Doe)' => '名＋姓（例： John Doe）',
    'Fixed format (dd/mm/yyyy) - Example: {example}' => '固定フォーマット（dd / mm / yyyy） - 例：{example}',
    'Friendship' => 'フレンドシップ',
    'General' => '基本',
    'General Settings' => '基本設定',
    'Global topics can be used by all users in all Spaces. They make it easier for you to define consistent keywords throughout your entire network. If users have already created topics in Spaces, you can also convert them to global topics here.' => 'グローバルトピックは、すべてのスペースのすべてのユーザーが使用できます。これによりネットワーク全体で一貫したキーワードを簡単に定義できます。ユーザーがすでにスペースでトピックを作成している場合は、ここでそれらをグローバルトピックに変換することもできます。',
    'HTML tracking code' => 'HTMLのトラッキングコード',
    'Here you can configurate the registration behaviour and additinal user settings of your social network.' => 'ここでは、ソーシャルネットワークの登録動作や追加のユーザー設定を行うことができます。',
    'Here you can configure basic settings of your social network.' => 'ここでは、ソーシャルネットワークの基本的な設定を行うことができます。',
    'Icon upload' => 'アイコンのアップロード',
    'In order for links to open in the app on mobile devices, rather than in the mobile browser, all links (e.g. notification emails) need to be routed through the HumHub proxy server.' => 'リンクをモバイル ブラウザーではなくモバイル デバイスのアプリで開くには、すべてのリンク (通知メールなど) を HumHub プロキシ サーバー経由でルーティングする必要があります。',
    'Inserted script tags must contain a nonce. e.g. {code}' => '挿入されたスクリプト タグにはノンスが含まれている必要があります。
例 : {code}',
    'Last visit' => '最後の訪問',
    'Logo upload' => 'ロゴのアップロード',
    'Mail Transport Type' => 'メールのトランスポートタイプ',
    'Maintenance mode' => 'メンテナンスモード',
    'Maintenance mode restricts access to the platform and immediately logs out all users except Admins.' => 'メンテナンス モードではプラットフォームへのアクセスが制限され、管理者以外のすべてのユーザーが直ちにログアウトされます。',
    'Maximum allowed age for logs.' => 'ログの最大許容寿命',
    'Maximum upload file size (in MB)' => 'アップロードするファイルの最大サイズ（MB単位）',
    'Mobile App' => 'モバイルアプリ',
    'Name of the application' => 'ソーシャルネットワークの名前',
    'No Delivery (Debug Mode, Save as file)' => '配信はありません (デバッグ モード、ファイルとして保存)',
    'No Proxy Hosts' => 'プロキシホストがない',
    'Notification Settings' => '通知の設定',
    'Old logs can significantly increase the size of your database while providing little information.' => '古いログは、ほとんど情報を提供しないでデータベースのサイズを大幅に増やすことができます。',
    'Optional. Default reply address for system emails like notifications.' => 'オプション。 通知などのシステム メールのデフォルトの返信アドレス。',
    'PHP (Use settings of php.ini file)' => 'PHP (php.ini ファイルの設定を使用)',
    'PHP reported a maximum of {maxUploadSize} MB' => 'PHPの最大ファイル容量は {maxUploadSize}MBに設定されている',
    'Password' => 'パスワード',
    'Please enable <a href="{url}" target="_blank">Pretty URLs</a> for proper working of the well-known files.' => 'よく知られているファイルが正しく動作するには、<a href="{url}" target="_blank">Pretty URLs</a> を有効にしてください。',
    'Port' => 'ポート',
    'Port number' => 'ポート番号',
    'Provider Name' => 'プロバイダ名',
    'Regular expression by which the link match will be checked.' => 'リンクの一致をチェックする正規表現。',
    'Save' => '保存',
    'Save & Test' => '保存してテストメールを送信',
    'Saved and sent test email to: {address}' => '保存してテストメールを送信：{address}',
    'Server' => 'サーバー',
    'Show introduction tour for new users' => '新規ユーザーのための機能紹介ツアーを表示',
    'Show user profile post form on dashboard' => 'ユーザープロフィール投稿をダッシュボードに表示する',
    'Sort by creation date' => '作成日順で並べる',
    'Sort by update date' => '更新日順で並べる',
    'Test message' => 'テスト メッセージ',
    'Theme' => 'テーマ',
    'These settings refer to advanced topics of your social network.' => 'ここでは、ソーシャルネットワークの高度な設定を行うことができます。',
    'These settings refer to the appearance of your social network.' => 'ここでは、ソーシャルネットワークの外観の設定を行うことができます。',
    'Topic has been deleted!' => 'トピックを削除しました',
    'Topics' => 'トピック',
    'URL to the file {fileNameLink}' => 'ファイルへの URL {fileNameLink}',
    'Url Pattern' => 'URL パターン',
    'Use %url% as placeholder for URL. Format needs to be JSON. (e.g. http://www.youtube.com/oembed?url=%url%&format=json)' => 'URLのプレースホルダに %url% を使用します。書式はJSONである必要があります。（例：http://www.youtube.com/oembed?url=%url%&amp;format=json）',
    'Use SMTPS' => 'SMTPSを使う',
    'Use X-Sendfile for File Downloads' => 'ファイルダウンロードのためにX-Sendfileを使用する',
    'User' => 'ユーザー',
    'User Display Name' => 'ユーザー表示名',
    'User Display Name Subtitle' => 'ユーザー表示名 サブタイトル',
    'User Settings' => 'ユーザー設定',
    'Username' => 'ユーザー名',
    'Username (e.g. john)' => 'ユーザー名（例：john）',
    'Well-known file {fileName}' => 'よく知られているファイル {fileName}',
    'Well-known files' => 'Well-known ファイル',
    'You can add a statistic code snippet (HTML) - which will be added to all rendered pages.' => 'すべてのレンダリングされたページに追加される統計HTMLコードスニペットを追加できます。',
    'You can find more configuration options here:' => 'ここでその他の構成オプションを見つけることができます。',
    'You\'re using no icon at the moment. Upload your logo now.' => '現在、アイコンを使用していません。今すぐアイコンをアップロードしてください。',
    'You\'re using no logo at the moment. Upload your logo now.' => '現在、ロゴを使用していません。今すぐロゴをアップロードしてください。',
    'e.g. 25 (for SMTP) or 465 (for SMTPS)' => '例：25 (SMTP) or 465 (SMTPS)',
    'e.g. localhost' => '例：localhost',
    'e.g. smtps://user:pass@smtp.example.com:port' => '例：smtps://user:pass@smtp.example.com:port',
    'never' => 'なし',
    'A dedicated image can be uploaded in the Appearance admin page.' => '',
    'Accent color' => '',
    'Caches flushed successfully' => '',
    'Cannot compile SCSS to CSS:' => '',
    'Custom SCSS' => '',
    'Danger color' => '',
    'Dark color' => '',
    'Domain URLs to whitelist, separated by comma' => '',
    'E.g. URLs of the SSO service providers' => '',
    'Email Header Image' => '',
    'Flush Caches' => '',
    'Info color' => '',
    'Invalid URL format: {domain}' => '',
    'Light color' => '',
    'Login Background' => '',
    'Primary color' => '',
    'Recommended minimum height: {recommendedMinHeight}px (minimum {minWidth} x {minHeight} px).' => '',
    'Recommended size: {recommendedWidth} × {recommendedHeight}px (minimum {minWidth} x {minHeight} px).' => '',
    'Secondary color' => '',
    'Show network name instead of logo in email header' => '',
    'Success color' => '',
    'Theme customization' => '',
    'Use Sassy CSS syntax (SCSS)' => '',
    'Warning color' => '',
];

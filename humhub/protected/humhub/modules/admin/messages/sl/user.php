<?php
return [
  '<strong>Information</strong>' => '<strong> informacije </strong>',
  '<strong>Profile</strong> Permissions' => '<strong>Dovoljenja</strong> profila',
  '<strong>Settings</strong> and Configuration' => '<strong> Nastavitve </strong> in konfiguracija',
  '<strong>User</strong> administration' => '<strong> Uporabnik </strong> administracija',
  '<strong>Warning:</strong> All individual profile permission settings are reset to the default values!' => '<strong> OPOZORILO: </strong> Vse nastavitve dovoljenja za posamezne profile se ponastavijo na privzete vrednosti!',
  'About the account request for \'{displayName}\'.' => 'O zahtevi računa za \'{displayName}\'.',
  'Accept user: <strong>{displayName}</strong> ' => 'Sprejmi uporabnika: <strong> {displayName} </strong>',
  'Account' => 'Račun',
  'Account Request for \'{displayName}\' has been approved.' => 'Zahteva računa za \'{displayName}\' je odobrena.',
  'Account Request for \'{displayName}\' has been declined.' => 'Zahteva računa za \'{displayName}\' je zavrnjena.',
  'Actions' => 'Dejanja',
  'Active users' => 'Aktivni uporabniki',
  'Add Groups...' => 'Dodaj skupine ...',
  'Add Parent Group' => '',
  'Add Subgroup(s)' => '',
  'Add new category' => 'Dodaj novo kategorijo',
  'Add new field' => 'Dodaj novo polje',
  'Add new group' => 'Dodaj novo skupino',
  'Add new members...' => 'Dodaj nove člane ...',
  'Add new user' => 'Dodaj novega uporabnika',
  'Administrator group could not be deleted!' => 'Skrbniške skupine ni bilo mogoče izbrisati!',
  'All existing Profile Topics will be converted to Global Topics.' => 'Vse obstoječe teme profila bodo pretvorjene v globalne teme.',
  'All open registration invitations were successfully deleted.' => 'Vsa odprta vabila za registracijo so bila uspešno izbrisana.',
  'All open registration invitations were successfully re-sent.' => 'Vsa odprta vabila za registracijo so bila uspešno ponovno nameščena.',
  'All the personal data of this user will be irrevocably deleted.' => 'Vsi osebni podatki tega uporabnika bodo nepreklicno izbrisani.',
  'Allow' => 'Dovoli',
  'Allow individual topics on profiles' => 'Dovoli posamezne teme na profilih',
  'Allow users to block each other' => 'Omogočajo, da se medsebojno blokirajo',
  'Allow users to set individual permissions for their own profile?' => 'Uporabnikom omogočajo nastavitev posameznih dovoljenj za svoj profil?',
  'Allow visitors limited access to content without an account (Adds visibility: "Guest")' => 'Omogočite obiskovalcem omejen dostop do vsebine brez računa (doda vidljivost: "Gost")',
  'Applied to new or existing users without any other group membership.' => 'Uporablja se za nove ali obstoječe uporabnike brez drugega članstva v skupini.',
  'Apply' => 'Prijava',
  'Approve' => 'Odobriti',
  'Approve all selected' => 'Odobri vse izbrane',
  'Are you really sure that you want to disable this user?' => 'Ste res prepričani, da želite tega uporabnika onemogočiti?',
  'Are you really sure that you want to enable this user?' => 'Ste res prepričani, da želite omogočiti tega uporabnika?',
  'Are you really sure that you want to impersonate this user?' => 'Ste res prepričani, da želite predstaviti tega uporabnika?',
  'Are you really sure? The selected users will be approved and notified by e-mail.' => 'Ste res prepričani? Izbrani uporabniki bodo odobreni in obveščeni po e-pošti.',
  'Are you really sure? The selected users will be deleted and notified by e-mail.' => '',
  'Are you really sure? The selected users will be notified by e-mail.' => '',
  'Are you really sure? Users who are not assigned to another group are automatically assigned to the default group.' => '',
  'Are you sure that you want to delete following user?' => '',
  'Cancel' => 'Odpovej',
  'Cannot resend invitation email!' => '',
  'Click here to review' => '',
  'Confirm user deletion' => '',
  'Convert' => '',
  'Convert Profile Topics' => '',
  'Could not approve the user!' => '',
  'Could not decline the user!' => '',
  'Could not load category.' => '',
  'Could not send the message to the user!' => '',
  'Create new group' => '',
  'Create new profile category' => '',
  'Create new profile field' => '',
  'Deactivate individual profile permissions?' => '',
  'Decline' => '',
  'Decline & delete user: <strong>{displayName}</strong>' => '',
  'Decline all selected' => '',
  'Default' => 'Privzeto',
  'Default Profile Permissions' => '',
  'Default Sorting' => '',
  'Default content of the email when sending a message to the user' => '',
  'Default content of the registration approval email' => '',
  'Default content of the registration denial email' => '',
  'Default group can not be deleted!' => '',
  'Default user idle timeout, auto-logout (in seconds, optional)' => '',
  'Default user profile visibility' => '',
  'Delete' => 'Briši',
  'Delete All' => '',
  'Delete all contributions of this user' => '',
  'Delete invitation' => '',
  'Delete invitation?' => '',
  'Delete pending registrations?' => '',
  'Delete spaces which are owned by this user' => '',
  'Deleted' => '',
  'Deleted invitation' => '',
  'Deleted users' => '',
  'Disable' => 'Onemogoči',
  'Disabled' => 'Onemogočeno',
  'Disabled users' => '',
  'Do not change placeholders like {displayName} if you want them to be automatically filled by the system. To reset the email content fields with the system default, leave them empty.' => '',
  'Do you really want to delete pending registrations?' => '',
  'Do you really want to re-send the invitations to pending registration users?' => '',
  'Edit' => 'Uredi',
  'Edit category' => '',
  'Edit profile category' => '',
  'Edit profile field' => '',
  'Edit user: {name}' => '',
  'Email all selected' => '',
  'Enable' => 'Omogoči',
  'Enable individual profile permissions' => '',
  'Enabled' => 'Omogoči',
  'First name' => '',
  'Group Manager' => '',
  'Group Type' => '',
  'Group not found!' => '',
  'Group user not found!' => '',
  'Groups' => 'Skupine',
  'Hello {displayName},

Your account creation is under review.
Could you tell us the motivation behind your registration?

Kind Regards
{AdminName}

' => '',
  'Hello {displayName},

Your account has been activated.

Click here to login:
{loginUrl}

Kind Regards
{AdminName}

' => '',
  'Hello {displayName},

Your account request has been declined.

Kind Regards
{AdminName}

' => '',
  'Here you can create or edit profile categories and fields.' => '',
  'Hide online status of users' => '',
  'If enabled, the Group Manager will need to approve registration.' => '',
  'If individual profile permissions are not allowed, the following settings are unchangeable for all users. If individual profile permissions are allowed, the settings are only set as defaults that users can customise. The following entries are then displayed in the same form in the users profile settings:' => '',
  'If this option is not selected, the ownership of the spaces will be transferred to your account.' => '',
  'Impersonate' => '',
  'Information 1' => '',
  'Information 2' => '',
  'Information 3' => '',
  'Invisible' => '',
  'Invite new people' => '',
  'Invite not found!' => '',
  'Last login' => 'Zadnja prijava',
  'Last name' => '',
  'List pending registrations' => '',
  'Make the group selectable at registration.' => '',
  'Manage group: {groupName}' => '',
  'Manage groups' => '',
  'Manage profile attributes' => '',
  'Member since' => '',
  'Members' => '',
  'Members can invite external users by email' => '',
  'Members can invite external users by link' => '',
  'Message' => '',
  'New approval requests' => '',
  'New users can register' => '',
  'No' => '',
  'No users were selected.' => '',
  'No value found!' => '',
  'None' => '',
  'Normal' => '',
  'Not visible' => '',
  'One or more user needs your approval as group admin.' => '',
  'Only applicable when limited access for non-authenticated users is enabled. Only affects new users.' => '',
  'Overview' => '',
  'Parent Group' => '',
  'Parent group is required!' => '',
  'Password' => '',
  'Pending approvals' => '',
  'Pending user approvals' => '',
  'People' => 'Ljudje',
  'Permanently delete' => '',
  'Permissions' => '',
  'Post-registration approval required' => '',
  'Prioritised User Group' => '',
  'Profile Permissions' => '',
  'Profiles' => '',
  'Protected' => '',
  'Protected group can not be deleted!' => '',
  'Re-send to all' => '',
  'Remove from group' => '',
  'Resend invitation email' => '',
  'Resend invitation?' => '',
  'Resend invitations?' => '',
  'Save' => 'Shrani',
  'Search by name, email or id.' => '',
  'Select Groups' => '',
  'Select a prioritised group whose members are displayed before all others when the sorting option \'Default\' is selected. The users within the group and the users outside the group are additionally sorted by their last login.' => '',
  'Select the profile fields you want to add as columns' => '',
  'Select which user information should be displayed in the \'People\' overview. You can select any profile fields, even those you have created individually. ' => '',
  'Send' => '',
  'Send & decline' => '',
  'Send & save' => '',
  'Send a message' => '',
  'Send a message to <strong>{displayName}</strong> ' => '',
  'Send invitation email' => '',
  'Send invitation email again?' => '',
  'Send notifications to users when added to or removed from the group.' => '',
  'Settings' => 'Nastavitve',
  'Show group selection at registration' => '',
  'Subgroup' => '',
  'Subgroup(s)' => '',
  'Subject' => '',
  'The Space memberships of all group members will be updated. This may take up to several minutes.' => '',
  'The default user idle timeout is used when user session is idle for a certain time. The user is automatically logged out after this time.' => '',
  'The following list contains all pending sign-ups and invites.' => '',
  'The following list contains all registered users awaiting an approval.' => '',
  'The message has been sent by email.' => '',
  'The registration was approved and the user was notified by email.' => '',
  'The registration was declined and the user was notified by email.' => '',
  'The registrations were approved and the users were notified by email.' => '',
  'The registrations were declined and the users were notified by email.' => '',
  'The selected invitations have been successfully deleted!' => '',
  'The selected invitations have been successfully re-sent!' => '',
  'The user cannot be removed from this Group!' => '',
  'The user cannot be removed from this Group, as users are required to be assigned to at least one Group.' => '',
  'The user is the owner of these spaces:' => '',
  'The users were notified by email.' => '',
  'This option allows you to determine whether users may set individual permissions for their own profiles.' => '',
  'This overview contains a list of each registered user with actions to view, edit and delete users.' => '',
  'This user group includes {count} additional members from the subgroup(s). To view the subgroups, check the Settings section.' => '',
  'This user owns no spaces.' => '',
  'Type' => '',
  'Unapproved' => '',
  'User deletion process queued.' => '',
  'User is already a member of this group.' => '',
  'User not found!' => '',
  'Users can be assigned to different groups (e.g. teams, departments etc.) with specific standard spaces, group managers and permissions.' => '',
  'Using this option any contributions (e.g. contents, comments or likes) of this user will be irrevocably deleted.' => '',
  'View profile' => '',
  'Visible for members only' => '',
  'Visible for members+guests' => '',
  'Will be used as a filter in \'People\'.' => '',
  'Yes' => '',
  'You can only delete empty categories!' => '',
  'You cannot delete yourself!' => '',
  'never' => '',
  '{nbMsgSent} already sent' => '',
];

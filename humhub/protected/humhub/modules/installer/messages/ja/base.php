...
return [
    ...
    '<strong>Pretty URLs</strong>' => '<strong>きれいなURL</strong>。',
    ...
    'Allow access for non-registered users to public content (guest access)' => 'ユーザー登録をしていない人でも公開コンテンツにアクセスできる（ゲストアクセス）',
    ...
    'For more information on this topic, please refer to our <a href="{link}" target="_blank">documentation</a>.' => 'このトピックの詳細については、<a href="{link}" target="_blank">ドキュメント</a>を参照してください。',
    ...
    'HumHub uses cron jobs to execute scheduled or to run potentially heavy tasks such as sending out email summaries and notifications or optimizing the search index.' => 'HumHubはcronジョブを使用して、スケジュールされたタスクを実行したり、電子メールの要約や通知の送信、検索インデックスの最適化など、潜在的に重いタスクを実行したりします。',
    'I want to use HumHub for:' => 'あなたがHumHubを利用する形態は：',
    'In the installation folder, locate the <strong>.env.example</strong> file and copy its contents into the <strong>.env</strong> file. Next, find the <strong>Pretty URLs</strong> block and uncomment it by removing the <strong>#</strong> symbol.' => 'インストールフォルダ内の<strong>.env.example</strong>ファイルを見つけ、その内容を<strong>.env</strong>ファイルにコピーします。次に、<strong>Pretty URLs</strong>ブロックを探し、<strong>#</strong>記号を削除してコメントを解除します。',
    'Initializing database...' => 'データベースを構築しています。',
    'Installation Example:' => 'インストール例：',
    'Language and timezone are automatically set according to the user\'s browser settings. If that is not possible, the default settings are used.' => '言語とタイムゾーンはユーザーのブラウザ設定に応じて自動的に設定されます。それが不可能な場合はデフォルト設定が使用されます。',
...
<code>{user}</code>',
    ...
    'Registered members can invite new users via link' => '登録メンバーはリンクを介して新しいユーザーを招待できます',
...
ドキュメントに記載されているジョブスケジューリングの設定に問題がある場合は、サーバー管理者にサポートを依頼してください。',
    ...
    'You\'re almost done. In this step you have to fill out the form to create an admin account. With this account you can manage the whole network.' => 'これで最後です。管理者アカウントを作成してください。このアカウントがSNSを管理します。',

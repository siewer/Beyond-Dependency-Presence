...
return [
    ...
    '<strong>Pretty URLs</strong>' => '<strong>Schöne URLs</strong>',
    '<strong>Scheduled jobs</strong>' => '<strong>Scheduled jobs</strong>',
    '<strong>System</strong> Check' => '<strong>System</strong> Check',
    '<strong>Welcome</strong> to HumHub<br>Your Social Network Toolbox' => '<strong>Willkommen</strong> zu HumHub<br>Dem Werkzeugkasten für dein Soziales Netzwerk',
    ...
    'HumHub uses cron jobs to execute scheduled or to run potentially heavy tasks such as sending out email summaries and notifications or optimizing the search index.' => 'HumHub verwendet Cron Jobs, um geplante oder potenziell schwere Aufgaben auszuführen, wie das Versenden von E-Mail-Zusammenfassungen und -Benachrichtigungen oder die Optimierung des Suchindexes.',
    'I want to use HumHub for:' => 'Ich möchte HumHub benutzen für:',
    'In the installation folder, locate the <strong>.env.example</strong> file and copy its contents into the <strong>.env</strong> file. Next, find the <strong>Pretty URLs</strong> block and uncomment it by removing the <strong>#</strong> symbol.' => 'Wechsle im Installationsordner zur Datei <strong>.env.example</strong> und kopiere deren Inhalt in die Datei <strong>.env</strong>. Suche anschließend den Abschnitt <strong>Pretty URLs</strong> und hebe die Auskommentierung auf, indem du das <strong>#</strong>-Symbol entfernst.',
    ...
    'Open the crontab of HumHub/PHP process user e.g. <code>{user}</code>.' => 'Öffne die Crontab des HumHub/PHP-Prozessbenutzers, z.B. <code>{user}</code>.',
    ...
    'Registered members can invite new users via link' => 'Registrierte Mitglieder können neue Benutzer per Link einladen',

...
final class UnsubscribedHelperTest extends MauticMysqlTestCase
{
    ...
    public function testStatsWithFilters(
        ...
        $this->unsubscribedHelper->generateStats($fromDateTime, $toDateTime, $options, $statCollection);
    ...
    public static function statsFilterProvider(): \Generator
    {
        yield 'admin can view all stats' => [
            ...
            'dateTo'            => '+1 day',
            'expectedCount'     => 2,
            'message'           => 'Admin should see all 2 unsubscribe records',
        ];
    ...
    public function testOnlyUnsubscribeReasonIncluded(): void
    {
        // Add a bounced contact
        $bouncedContact = new DoNotContact();
        $bouncedContact->setLead($this->lead1);
        $bouncedContact->setChannel('email');
        $bouncedContact->setChannelId($this->email->getId());
        $bouncedContact->setReason(DoNotContact::BOUNCED);
        $bouncedContact->setDateAdded(new \DateTime());
        $this->em->persist($bouncedContact);
        $this->em->flush();
        ...
        // Should still be 2, not 3, because bounced should not be counted
        $this->assertSame(2, $sum, 'Should only count UNSUBSCRIBED reason, not BOUNCED');
    }
    ...
    private function createLeadsAndStats(): void
    {
        ...
        $dnc2->setReason(DoNotContact::UNSUBSCRIBED);

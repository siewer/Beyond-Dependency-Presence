...
module.exports = function(responderId, config, ss) {
  ...
  return {
    ...
    interfaces: function(middleware) {
      ...
      // Get request handler
      var request = require('./request')(ss, middleware);
      return {
        ...
        websocket: function(msg, meta, send) {
          ...
          ss.log.debug('↪'.cyan, msgLogName, req.method); //TODO debug('↪')
          ...
          handleError = function(e) {
            ...
            ss.log.error('↩'.red, msgLogName, req.method, e.message.red); //TODO debug('↩')
            if (e.stack) {
              ss.log.error(e.stack.split('\n').splice(1).join('\n'));
            }
          ...
          try {
            return request(req, function(err, response) {
              ...
              ss.log.debug('↩'.green, msgLogName, req.method, ('(' + timeTaken + 'ms)').grey);
              return send(JSON.stringify(obj));
            });
          } catch (e) {

...
func generateSessionVars() []byte {
	...
	for name, v := range sql.GetSessionVars() {
		...
		// Skip platform-specific default values that would cause the
		// generated doc to differ across OS/arch.
		...
		vars = append(vars, sessionVar{
			...
			clusterSettingName: v.ClusterSettingName,
		})
	}
	...
	b.WriteString("<th>Cluster Setting</th>\n")
	...
	for _, v := range vars {
		...
		if v.clusterSettingName != "" {
			clusterSetting = "<code>" + v.clusterSettingName + "</code>"
		}
		fmt.Fprintf(b,

...
type TaskInfo struct {
	...
	// ExecutorFunc is the Go function name (e.g., "validateCluster"), not the registered executor name (e.g., "validate-cluster").
	ExecutorFunc    string                 `json:"executor_func,omitempty"`
	ResultProcessor string                 `json:"result_processor,omitempty"`
	Params          []string               `json:"params,omitempty"`
	ForkTasks       []string               `json:"fork_tasks,omitempty"`
	Join            string                 `json:"join,omitempty"` // For ForkTask: the join point where branches converge
...
func SerializeTask(task Task, executorRegistry map[string]*Executor) TaskInfo {
	...
	case TaskTypeExecution:
		...
		taskInfo.ExecutorFunc = execTask.GetExecutorName(executorRegistry)
	...
	case TaskTypeFork:
		...
		if forkTask.Join != nil {
			taskInfo.Join = forkTask.Join.Name()
		}

	case TaskTypeForkJoinTask:
		// ForkJoinTask is a synchronization point with no type-specific fields.
		// The base fields (name, type) are sufficient.

	case TaskTypeConditionTask:
		conditionTask := task.(*ConditionTask)
		taskInfo.ExecutorFunc = conditionTask.GetExecutorName(executorRegistry)
	...
	case TaskTypeCallbackTask:
		...
		taskInfo.ExecutorFunc = callbackTask.GetExecutionFnName(executorRegistry)
		taskInfo.ResultProcessor = callbackTask.GetResultProcessorFnName(executorRegistry)
		taskInfo.Params = getTaskNames(callbackTask.Params)
	...
	case TaskTypeChildPlanTask:
		...
		taskInfo.ExecutorFunc = childTask.GetExecutorName(executorRegistry)

...
func getSpanStats(
	...
	var stats spanStats
	var rangeCount, approximateDiskBytes, liveBytes, totalBytes gosql.NullInt64
	err := conn.QueryRow(
		`SELECT `+
	...
	require.NoError(t, err)
	// NULLs are returned when the span no longer has any ranges (e.g.,
	// after a table is fully dropped and GC'd). Treat as zeros — the
	// caller's progress check will see this as fully reclaimed.
	stats.rangeCount = int(rangeCount.Int64)
	stats.approximateDiskBytes = approximateDiskBytes.Int64
	stats.liveBytes = liveBytes.Int64

...
// or equal to maxD. If the actual distance is greater than maxD, it returns maxD + 1.
...
// distances less than or equal to maxDist. If the actual distance is greater than maxDist,
// it returns maxDist + 1. This approach uses the Wagner-Fischer algorithm and is adapted from the
...
func LevenshteinLessEqualDistanceWithCost(
	...
	// Use utf8.RuneCountInString to count characters properly for multi-byte strings.
	lenS, lenT := utf8.RuneCountInString(source), utf8.RuneCountInString(target)
	...
	if maxDist >= 0 {
		...
		subCost = min(subCost, insCost+delCost)
		maxTheoDist := minTheoDist + subCost*min(lenS, lenT)
	...
	// If maxDist was set to -1 due to loose bounds, fall back to regular levenshtein.
	if maxDist < 0 {
		return LevenshteinDistanceWithCost(source, target, insCost, delCost, subCost)
	}

	// Converting to a slice of runes allows us to count multi-byte characters
	// properly.
	// TODO(mgartner): We should be able to eliminate these allocations in some cases, e.g., when
	// the string contains only single-byte characters.
	s, t := []rune(source), []rune(target)

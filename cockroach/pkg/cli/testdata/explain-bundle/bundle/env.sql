...
--   diagnostics.reporting.send_crash_reports.enabled = true  (send crash and panic reports)
...
--   kv.closed_timestamp.follower_reads.enabled = true  (allow (all) replicas to serve consistent historical reads based on closed timestamp information)
...
--   kv.raft_log.synchronization.disabled = false  (set to true to disable synchronization on Raft log writes to persistent storage. Setting to true risks data loss or data corruption on server crashes. The setting is meant for internal testing only and SHOULD NOT be used in production.)
...
--   kv.range_merge.queue.enabled = true  (whether the automatic merge queue is enabled)
--   kv.range_merge.queue_interval = 5s  (how long the merge queue waits between processing replicas)
--   kv.range_split.by_load.enabled = true  (allow automatic splits of ranges based on where load is concentrated)
...
--   kv.transaction.parallel_commits.enabled = true  (if enabled, transactional commits will be parallelized with transactional writes)
...
--   kv.transaction.write_pipelining.enabled = true  (if enabled, transactional writes are pipelined through Raft consensus)
...
--   server.clock.forward_jump_check.enabled = false  (if enabled, forward clock jumps > max_offset/2 will cause a panic)
...
--   server.oidc_authentication.autologin.enabled = false  (if true, logged-out visitors to the DB Console will be automatically redirected to the OIDC login endpoint (this feature is experimental))
...
--   server.oidc_authentication.tls_insecure_skip_verify.enabled = false  (if true, TLS certificate verification is skipped for connections to the OIDC provider (insecure) (this feature is experimental))
...
--   server.shutdown.transactions.timeout = 10s  (the server will wait for at least this amount of time for active queries to finish)
...
--   sql.distsql.flow_stream.timeout = 10s  (amount of time incoming streams wait for a flow to be set up before erroring out)
...
--   sql.metrics.statement_details.dump_to_logs.enabled = false  (dump collected statement statistics to node logs when periodically cleared)
...
--   sql.log.all_statements.enabled = false  (set to true to enable logging of executed statements)
...
--   trace.debug_http_endpoint.enabled = false  (if set, traces for recent requests can be seen at https://<ui>/debug/requests)

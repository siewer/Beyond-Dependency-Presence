	...
	ast.Inspect(f, func(n ast.Node) bool {
		...
		if nameExpr := findField(cl, "Name"); nameExpr != nil {
			if ident, isIdent := nameExpr.(*ast.Ident); isIdent {
				if fnName, paramIdx := findEnclosingFuncParam(
					f, cl, ident.Name,
				); fnName != "" {
	...
	for {
		...
		ast.Inspect(f, func(n ast.Node) bool {
			...
			if ident, ok := arg.(*ast.Ident); ok {
				if fnName, pIdx := findEnclosingFuncParam(
					f, call, ident.Name,
				); fnName != "" {
		...
		ast.Inspect(f, func(n ast.Node) bool {
			...
			for i, lhs := range assign.Lhs {
				if id, ok := lhs.(*ast.Ident); ok &&
					id.Name == v.Name && i < len(assign.Rhs) {
...
func (r *Result) MarshalText() []byte {
	...
	// Sort exact matches by name for deterministic output.
	...
	// Sort patterns by format string.
	sortedPatterns := make([]sprintfPattern, len(r.Patterns))
	copy(sortedPatterns, r.Patterns)
	sort.Slice(sortedPatterns, func(i, j int) bool {
		return sortedPatterns[i].format < sortedPatterns[j].format
	})
	for _, p := range sortedPatterns {
		fmt.Fprintf(&buf, "P\t%s\t%s:%d\n", p.format, p.source.File, p.source.Line)
	}

	// Sort prefixes by prefix string.
	sortedPrefixes := make([]struct {
		Prefix string
		Source Source
	}, len(r.Prefixes))
	copy(sortedPrefixes, r.Prefixes)

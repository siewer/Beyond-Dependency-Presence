...
func (r *Replica) applySnapshotRaftMuLocked(
	...
	// By default, the snapshot is applied to Pebble as a single ingestion, which
	// includes all the SSTs already prepared in the inSnap, and additional ones
	// generated by snapWriter below. If the snapshot is small/simple, everything
	// is written as a batch instead, for performance reasons.
	applyAsBatch := len(inSnap.externalSSTs)+len(inSnap.sharedSSTs) == 0 && /* simple */
	...
	// Stage the truncation, so that in-memory state reflects an empty log.
	ls := r.asLogStorage()
	...
	if applyAsBatch {
		// Admission control wants the writeBytes to be roughly equivalent to the
		// bytes in the SST when these writes are eventually flushed. We use the SST
		// size of the incoming snapshot as that approximation. We've written
		// additional SSTs to clear some data earlier in this method, but we ignore
		// those since the bulk of the data is in the incoming snapshot.

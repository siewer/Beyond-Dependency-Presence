...
func DeleteRange(
	...
	if args.UseRangeTombstone || hasPredicate {
...
func deleteRangeWithPredicate(
	...
	if !args.UseRangeTombstone {
		resumeSpan, err = storage.MVCCPredicateDeleteRangePointTombstones(ctx, readWriter,
		...
		// TODO(jeffswenson): Delete this code path when V26_1 is older than the
		// minimum supported binary version.
		desc := cArgs.EvalCtx.Desc()
		leftPeekBound, rightPeekBound := rangeTombstonePeekBounds(
			args.Key, args.EndKey, desc.StartKey.AsRawKey(), desc.EndKey.AsRawKey())

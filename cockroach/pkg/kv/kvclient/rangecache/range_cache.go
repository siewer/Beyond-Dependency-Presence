...
// between multiple EvictionTokens. If the request learns new information from
...
// information in case the EvictionToken was no longer up to date with the cache
// entry from whence it came. The leaseholder and range descriptor can be
...
func (et *EvictionToken) SyncTokenAndMaybeUpdateCache(
	...
	// cache. If it isn't, and the supplied range descriptor is newer than
	// what's in the cache, we simply evict the old descriptor and add the new
	// descriptor/lease pair. On the other hand, if the supplied range
...
func (et *EvictionToken) RandomizeLeaseholder(ctx context.Context) {
	if !et.Valid() {
		panic("trying to randomize leaseholder on an invalid token")
	}
	...
	// Don't modify this entry if the leaseholder in the underlying cache has
	// been changed already or if there aren't at least 2 replicas to choose
	// from.
	if !stillValid ||
		cachedEntry.lease.Replica != lh ||
		len(cachedEntry.desc.Replicas().VoterDescriptors()) <= 1 {
		return
	}

	newEntry := cachedEntry.randomizeLeaseholder(et.rdc.clock.Now())
	et.entry = newEntry
	et.rdc.swapEntryLocked(ctx, rawEntry, newEntry)
}
...
func (et *EvictionToken) MarkLeaseholderContacted() {
	...
	et.entry.updateLastLeaseholderContacted(et.rdc.clock.Now())
}
...
func (et *EvictionToken) SinceLeaseholderContacted() time.Duration {
	...
	return et.entry.sinceLeaseholderContacted(et.rdc.clock.Now())
}
...
// replica.
func (e *cacheEntry) randomizeLeaseholder(now time.Time) *cacheEntry {
	...
	// speculative leaseholder (e.lease may be empty, in which case any
	// randLease works).
...
func (e *cacheEntry) updateLastLeaseholderContacted(now time.Time) {
	e.lastLeaseholderContactedNanos.Store(now.UnixNano())
}

func (e *cacheEntry) sinceLeaseholderContacted(now time.Time) time.
	Duration {
	return time.Duration(now.UnixNano() - e.lastLeaseholderContactedNanos.Load())
}

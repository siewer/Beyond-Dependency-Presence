...
func TestNewExternalSSTReader(t *testing.T) {
	...
	fileStores := make([]storage.StoreFile, len(subdirs))
	...
	iter, err := storage.ExternalSSTReader(ctx, fileStores, nil, iterOpts)
	require.NoError(t, err)
...
func TestNewExternalSSTReaderFailure(t *testing.T) {
	...
	_, err = storage.ExternalSSTReader(ctx, fileStores, nil, iterOpts)
	require.Error(t, err)
	require.Equal(t,
		int64(0),

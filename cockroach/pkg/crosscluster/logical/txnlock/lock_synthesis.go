...
func (ls *LockSynthesizer) DeriveLocks(
	...
	for i, row := range rows {
		var err error
		rowLocks[i], err = ls.deriveLocks(ctx, row)
		if err != nil {
			return LockSet{}, err
		}
...
func (ls *LockSynthesizer) deriveLocks(
	...
	if !ok {
		// We return an error here because it indicates we were passed a row for a
		// table that was not included when the LockSynthesizer was initialized.
		return nil, errors.AssertionFailedf("unknown table %d", row.TableID)
	}
	return tc.deriveLocks(ctx, row)
}

// dependsOn returns true if row b must be applied before row a. It can be read
// as a `dependsOn` b.

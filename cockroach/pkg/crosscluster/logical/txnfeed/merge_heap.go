...
type mergeEntry struct {
	...
	// peekedKVs holds the remaining KVs from the current peeked event when it
	// is a KV event. As KVs are consumed, the slice is resliced forward.
	...
	// can compare entries without recomputing it.
...
func (h *mergeHeap) pop() *mergeEntry   { return heap.Pop(h).(*mergeEntry) }
func (h *mergeHeap) len() int           { return len(*h) }
...
// peekIsKVAt reports whether the next entry on the heap is a KV at the given
// timestamp. This is used to determine if more KVs from the same transaction
// remain in the heap.
func (h *mergeHeap) peekIsKVAt(ts hlc.Timestamp) bool {
	e := h.peek()
	return e != nil && e.peekTS == ts && e.isKV()
}

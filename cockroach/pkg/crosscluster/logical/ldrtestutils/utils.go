...
// ApplyLowLatencyReplicationSettings applies cluster settings that reduce
// latency for logical replication testing. sysRunner configures system-level
// settings, and appRunner configures application-level settings.
...
// WaitUntilReplicatedTime waits for a logical replication job to reach the
// target replicated time. The test will fail immediately if the job enters a
...
func WaitUntilReplicatedTime(
	...
	t.Logf("waiting for logical replication job %d to reach replicated time of %s",
		jobID, targetTime)
	testutils.SucceedsSoon(t, func() error {
		...
		if replicatedTime.Less(targetTime) {
			return errors.Newf(
				"logical replication job %d at %s, waiting to advance past %s",
				jobID, replicatedTime, targetTime)
		}
		return nil
	})
}

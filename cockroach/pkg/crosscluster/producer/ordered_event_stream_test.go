...
`)
	const numKeys = 20
	// Create split points for each table.
	srcTenant.SQL.Exec(t, `ALTER TABLE d.kv1 SPLIT AT VALUES (10)`)
	srcTenant.SQL.Exec(t, `ALTER TABLE d.kv2 SPLIT AT VALUES (10)`)
...
consumeStream:
	for {
		...
		if streamEvent.Checkpoint != nil {
			...
			// Ensure that span checkpoints don't go backwards and all spans
			// in a single checkpoint should have the same timestamp.
			var nextCheckpointTS hlc.Timestamp
			for i, resolvedSpan := range streamEvent.Checkpoint.ResolvedSpans {
				if i == 0 {
					nextCheckpointTS = resolvedSpan.Timestamp
					require.False(
						t,
						nextCheckpointTS.Less(frontierTS),
						"checkpoint timestamp went backwards: frontier %s -> checkpoint %s",
						frontierTS,
						nextCheckpointTS,
					)
				} else {
					require.True(
						t,
						resolvedSpan.Timestamp.Equal(nextCheckpointTS),
						"span %s has timestamp %s, expected %s. All spans in checkpoint should match",
						resolvedSpan.Span,

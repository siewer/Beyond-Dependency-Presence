...
func makeKV(key string, ts int64, value string) *kvpb.RangeFeedValue {
	return makeKVWithPrev(key, ts, value, "")
}

func makeKVWithPrev(key string, ts int64, value string, prevValue string) *kvpb.RangeFeedValue {
...
func TestDeepCopyRangeFeedValueDoesNotAliasCaller(t *testing.T) {
	...
	kv := makeKVWithPrev("original-key", 1, "v1", "prev1")
...
func TestOrderedBufferRandomizedSequence(t *testing.T) {
	...
	for i, skv := range streamKVs {
		...
		// Add prev values to some KVs for testing.
		if i%3 == 0 {
	...
	// Ensure (key, timestamp) uniqueness across KVs and DelRanges.
	type keyTs struct {
		key string
		ts  int64
	}
	...
	// For each checkpoint, read events and validate sorting, bounds, and exact counts.
	var delivered [][]kvpb.RangeFeedEvent
	for _, rts := range checkpoints {
		events := getAllEventsFromDisk(t, buf, ctx, rts)
		// Validate bounds and sorting.
	...
	// Validate exact counts against expected.

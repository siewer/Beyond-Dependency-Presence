...
func TestStatementStatisticsComputedColumnsMigration(t *testing.T) {
	...
	var (
		validationStmts = []string{
			...
			`SELECT max_retries FROM system.statement_statistics LIMIT 0`,
		}
		validationSchemas = []upgrades.Schema{
			...
			{Name: "max_retries", ValidationFn: upgrades.ColumnExists},
			{Name: "stmt_fp_ts_cov_counts", ValidationFn: upgrades.IndexExists},
			{Name: "execution_count_idx", ValidationFn: upgrades.DoesNotHaveIndex},
			{Name: "service_latency_idx", ValidationFn: upgrades.DoesNotHaveIndex},
			{Name: "cpu_sql_nanos_idx", ValidationFn: upgrades.DoesNotHaveIndex},
			{Name: "contention_time_idx", ValidationFn: upgrades.DoesNotHaveIndex},
			{Name: "total_estimated_execution_time_idx", ValidationFn: upgrades.DoesNotHaveIndex},
			{Name: "p99_latency_idx", ValidationFn: upgrades.DoesNotHaveIndex},
		}
	)
...
func TestTransactionStatisticsComputedColumnsMigration(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)

	clusterversion.SkipWhenMinSupportedVersionIsAtLeast(t, clusterversion.V26_2_AddTransactionStatisticsComputedColumns)
	...
	var (
		validationStmts = []string{
			`SELECT exec_sample_count FROM system.transaction_statistics LIMIT 0`,
			`SELECT svc_lat_sum FROM system.transaction_statistics LIMIT 0`,
			`SELECT cpu_sql_nanos_sum FROM system.transaction_statistics LIMIT 0`,
			`SELECT contention_time_sum FROM system.transaction_statistics LIMIT 0`,
			`SELECT svc_lat_sum_sq FROM system.transaction_statistics LIMIT 0`,
			`SELECT cpu_sql_nanos_sum_sq FROM system.transaction_statistics LIMIT 0`,
			`SELECT contention_time_sum_sq FROM system.transaction_statistics LIMIT 0`,
			`SELECT kv_cpu_time_nanos_sum FROM system.transaction_statistics LIMIT 0`,
			`SELECT kv_cpu_time_nanos_sum_sq FROM system.transaction_statistics LIMIT 0`,
			`SELECT admission_wait_time_sum FROM system.transaction_statistics LIMIT 0`,
			`SELECT admission_wait_time_sum_sq FROM system.transaction_statistics LIMIT 0`,
			`SELECT rows_read_sum FROM system.transaction_statistics LIMIT 0`,
			`SELECT rows_written_sum FROM system.transaction_statistics LIMIT 0`,
			`SELECT bytes_read_sum FROM system.transaction_statistics LIMIT 0`,
			`SELECT bytes_read_sum_sq FROM system.transaction_statistics LIMIT 0`,
			`SELECT max_retries FROM system.transaction_statistics LIMIT 0`,
			`SELECT commit_lat_sum FROM system.transaction_statistics LIMIT 0`,
			`SELECT commit_lat_sum_sq FROM system.transaction_statistics LIMIT 0`,
		}
		validationSchemas = []upgrades.Schema{
			{Name: "exec_sample_count", ValidationFn: upgrades.ColumnExists},
			{Name: "svc_lat_sum", ValidationFn: upgrades.ColumnExists},
			...
			{Name: "bytes_read_sum_sq", ValidationFn: upgrades.ColumnExists},
			{Name: "max_retries", ValidationFn: upgrades.ColumnExists},
			{Name: "commit_lat_sum", ValidationFn: upgrades.ColumnExists},
			{Name: "commit_lat_sum_sq", ValidationFn: upgrades.ColumnExists},
			{Name: "txn_fp_ts_cov_counts", ValidationFn: upgrades.IndexExists},
			{Name: "execution_count_idx", ValidationFn: upgrades.DoesNotHaveIndex},
			{Name: "service_latency_idx", ValidationFn: upgrades.DoesNotHaveIndex},
			{Name: "cpu_sql_nanos_idx", ValidationFn: upgrades.DoesNotHaveIndex},
			{Name: "contention_time_idx", ValidationFn: upgrades.DoesNotHaveIndex},
	...
	upgrades.InjectLegacyTable(ctx, t, s, systemschema.TransactionStatisticsTable,
		getOldTransactionStatisticsDescriptor)
	validateSchemaExists := func(expectExists bool) {
		upgrades.ValidateSchemaExists(
			ctx,
			t,
	...
	// Validate that the transaction_statistics table has the old schema.
	validateSchemaExists(false)
	// Run the upgrade.
	upgrades.Upgrade(
		t,
		sqlDB,
		clusterversion.V26_2_AddTransactionStatisticsComputedColumns,
		nil,   /* done */
		false, /* expectError */
	)
	// Validate that the table has new schema.
...
func getOldStatementStatisticsDescriptor() *descpb.TableDescriptor {
	...
	return &descpb.TableDescriptor{
		...
		Columns: []descpb.ColumnDescriptor{
			...
			// Note: columns 20-36 are intentionally omitted for the old descriptor
		},
		NextColumnID: 20,
		...
		Indexes: []descpb.IndexDescriptor{
			...
			// Old metric indexes that will be dropped by the migration.
			{
...
// getOldTransactionStatisticsDescriptor returns the system.transaction_statistics
// table descriptor that was being used before adding the new computed columns,
// covering index, and dropping old metric indexes.
func getOldTransactionStatisticsDescriptor() *descpb.TableDescriptor {
	...
	return &descpb.TableDescriptor{
		...
		Columns: []descpb.ColumnDescriptor{
			...
			{Name: "execution_count", ID: 9, Type: types.Int, Nullable: true, ComputeExpr: &executionCountComputeExpr},
			{Name: "service_latency", ID: 10, Type: types.Float, Nullable: true, ComputeExpr: &serviceLatencyComputeExpr},
			{Name: "cpu_sql_nanos", ID: 11, Type: types.Float, Nullable: true, ComputeExpr: &cpuSqlNanosComputeExpr},
			{Name: "contention_time", ID: 12, Type: types.Float, Nullable: true, ComputeExpr: &contentionTimeComputeExpr},
			{Name: "total_estimated_execution_time", ID: 13, Type: types.Float, Nullable: true, ComputeExpr: &totalEstimatedExecutionTimeExpr},
			{Name: "p99_latency", ID: 14, Type: types.Float, Nullable: true, ComputeExpr: &p99LatencyComputeExpr},
			// Note: columns 15-33 are intentionally omitted for the old descriptor
		},
		NextColumnID: 15,
		Families: []descpb.ColumnFamilyDescriptor{
			{
		...
		Indexes: []descpb.IndexDescriptor{
			...
			// Note: txn_fp_ts_cov_counts index is intentionally omitted
		},
		NextIndexID:      9,
		Privileges:       catpb.NewCustomSuperuserPrivilegeDescriptor(privilege.ReadData, username.NodeUserName()),
		NextMutationID:   1,

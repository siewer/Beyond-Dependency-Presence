...
// SetCheckDuplicates enables or disables duplicate key detection during flush.
// When enabled, the writer returns a DuplicateKeyError if duplicate keys are
// found after sorting. This should be enabled when building unique indexes.
func (s *Writer) SetCheckDuplicates(check bool) {
	s.checkDuplicates = check
}
...
func (s *Writer) flushBuffer(ctx context.Context) error {
	...
	if s.checkDuplicates {
		for i := 1; i < len(s.kv); i++ {
			if s.kv[i].Key.Compare(s.kv[i-1].Key) == 0 {
				// Callers will remap this error to a user facing error. We use the same
				// error that's generated by sst_batcher.go so callers don't have to
				// distinguish between errors coming from different sources.
				return kvserverbase.NewDuplicateKeyError(s.kv[i].Key.Key, s.kv[i].Value)
			}
		}
	}

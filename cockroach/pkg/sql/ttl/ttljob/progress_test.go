...
func setupCheckpointProgressTrackerForTest(
...
) (*checkpointProgressTracker, *cluster.Settings, func()) {
	...
	updater := &checkpointProgressTracker{
		job:                nil,
		jobID:              jobspb.JobID(12345), // Use a fake job ID for testing
		settings:           settings,
	...
	return updater, settings, cleanup
}
...
func TestCheckpointProgressUpdater(t *testing.T) {
	...
	ctx := context.Background()
	updater, settings, cleanup := setupCheckpointProgressTrackerForTest(ctx, t)
	defer cleanup()

	defer func() {
		if updater.stopFunc != nil {
			updater.terminateTracker()
		}
	}()
...
func TestCheckpointProgressUpdaterContents(t *testing.T) {
	...
	ctx := context.Background()
	updater, _, cleanup := setupCheckpointProgressTrackerForTest(ctx, t)
	defer cleanup()
...
func TestCheckpointProgressUpdaterRestartScenario(t *testing.T) {
	...
	// Test restart scenario with no existing progress initially
	...
	// Call initTracker without existing progress - should return empty
	completedSpans, err := updater.initTracker(ctx, nil, &settings.SV)
	require.NoError(t, err)
	require.Len(t, completedSpans, 0) // No spans stored yet
	...
	// Add some spans and verify they're tracked.
	currentSpans := updater.addCompleted(existingSpan1, existingSpan2, existingSpan3)
	...
	// Manually flush to jobfrontier to test persistence
	err = updater.flushCheckpointUpdate(ctx)
	require.NoError(t, err)

	// Create a new tracker with the same job ID to test persistence
	newUpdater := &checkpointProgressTracker{
		jobID:      updater.jobID,      // Same job ID
		internalDB: updater.internalDB, // Same database
	}

	// Load completed spans from storage - should find the persisted spans
	loadedSpans, err := newUpdater.loadCompletedSpansFromStorage(ctx)
	require.NoError(t, err)
	require.Len(t, loadedSpans, 3)
	require.Contains(t, loadedSpans, existingSpan1)
	require.Contains(t, loadedSpans, existingSpan2)
	require.Contains(t, loadedSpans, existingSpan3)

...
// Name for the TTL completed spans frontier in jobfrontier
const ttlCompletedSpansKey = "ttl_completed_spans"
...
func (t *checkpointProgressTracker) loadCompletedSpansFromStorage(
	ctx context.Context,
) ([]roachpb.Span, error) {
	var completedSpans []roachpb.Span

	err := t.internalDB.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
		resolvedSpans, found, err := jobfrontier.GetResolvedSpans(ctx, txn, t.jobID, ttlCompletedSpansKey)
		if err != nil {
			return err
		}
		if !found {
...
func (t *checkpointProgressTracker) initTracker(
	...
	// Add loaded spans to our in-memory SpanGroup
...
func (t *checkpointProgressTracker) updateProgressCache(
	...
	var incomingProcProgress jobspb.RowLevelTTLProcessorProgress
	if err := pbtypes.UnmarshalAny(&meta.BulkProcessorProgress.ProgressDetails, &incomingProcProgress); err != nil {
		return false, errors.Wrapf(err, "unable to unmarshal ttl progress details")
	}
	...
	if orig == nil {
		return false, errors.AssertionFailedf("cached progress does not contain RowLevelTTL details")
	}
	rowLevelTTL := protoutil.Clone(orig).(*jobspb.RowLevelTTLProgress)
	...
	// that's set, then return true to have the caller update the job record
	// immediately. This ensures we capture the final state when a processor
	// completes its work, rather than waiting for the next periodic checkpoint update.
...
// terminateTracker implements the progressTracker interface.
func (t *checkpointProgressTracker) terminateTracker() {
	if t.stopFunc != nil {
		t.stopFunc()
		t.stopFunc = nil
	}
...
func (t *checkpointProgressTracker) flushProgress(ctx context.Context) error {
	...
	var cachedRowLevelTTL *jobspb.RowLevelTTLProgress
	func() {
		t.mu.Lock()
		defer t.mu.Unlock()
...
func (t *checkpointProgressTracker) calculateCurrentFractionComplete(
	...
	return t.calculateRangeFractionComplete(ctx, completedSpans)
}

// addCompleted adds the given spans to the completed spans frontier with automatic
...
func (t *checkpointProgressTracker) flushCheckpointUpdate(ctx context.Context) error {
	...
	return t.internalDB.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
		// Create a frontier for the spans (using zero timestamps since TTL doesn't need timing)

...
`
	...
	t.Run("ValidSANIdentities_WithMapOption", func(t *testing.T) {
		// HBA entry with map option
		hbaConf := "host all all all cert map=sanmap"
		parsed, err := hba.ParseAndNormalize(hbaConf)
		require.NoError(t, err)
		require.Len(t, parsed.Entries, 1)

		// Create identity map for SAN identities
		// Maps SAN:DNS:*.example.com to database users
		identMapData := `
sanmap      /^SAN:DNS:([^.]+)\.example\.com$     \1
sanmap      /^SAN:URI:https://(.+)\.example\.com$  \1_uri
sanmap      SAN:IP:192.0.2.1                      admin_ip
`
		identConf, err := identmap.From(strings.NewReader(identMapData))
		require.NoError(t, err)
		...
		// Test with multiple SAN identities from a certificate
		sanIdentities := []string{
			"SAN:DNS:alice.example.com",
			"SAN:URI:https://service.example.com",
			"SAN:IP:192.0.2.1",
		}

		mappings, err := mapper(ctx, sanIdentities)
		require.NoError(t, err)
		require.Len(t, mappings, 3)

		// Verify DNS SAN mapping
		require.Equal(t, "SAN:DNS:alice.example.com", mappings[0].SystemIdentity)
		require.Equal(t, "alice", mappings[0].MappedUser.Normalized())

		// Verify URI SAN mapping
		require.Equal(t, "SAN:URI:https://service.example.com", mappings[1].SystemIdentity)
		require.Equal(t, "service_uri", mappings[1].MappedUser.Normalized())

		// Verify IP SAN mapping
		require.Equal(t, "SAN:IP:192.0.2.1", mappings[2].SystemIdentity)
		require.Equal(t, "admin_ip", mappings[2].MappedUser.Normalized())
	})

	t.Run("ValidSANIdentities_NoMapOption", func(t *testing.T) {
		// HBA entry with no "map" option - uses provided identities as-is
		hbaConf := "host all all all cert"
		parsed, err := hba.ParseAndNormalize(hbaConf)
		require.NoError(t, err)
		require.Len(t, parsed.Entries, 1)
		...
		// Test with SAN identities - should use them directly as usernames
		sanIdentities := []string{
			"SAN:DNS:alice.example.com",
			"SAN:URI:bob.example.com",
		}
		...
		// Without map option, SAN identities are used directly as database usernames
		require.Equal(t, "SAN:DNS:alice.example.com", mappings[0].SystemIdentity)
		require.Equal(t, "san:dns:alice.example.com", mappings[0].MappedUser.Normalized())
		require.Equal(t, "SAN:URI:bob.example.com", mappings[1].SystemIdentity)
		require.Equal(t, "san:uri:bob.example.com", mappings[1].MappedUser.Normalized())
	})
}

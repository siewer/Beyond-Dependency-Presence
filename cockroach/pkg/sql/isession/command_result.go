...
type internalCommandResult struct {
	...
	// err is the commands error. When assigning to err, use errors.CombineErrors
	// since the error may already contain a failure.
	err     error
	numRows int
	rows    []tree.Datums

	// resultBuffer is the owner of the `internalCommandResult`. It contains
	// ready results and a pool of resuable results.
...
func (i *internalCommandResult) GetFormatCode(colIdx int) (pgwirebase.FormatCode, error) {
	// Rows aren't serialized by the internal session, so this format code

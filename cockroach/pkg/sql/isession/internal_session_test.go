...
func TestInternalSessionPrepare(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		rows, err := session.ExecutePrepared(ctx, prepared, nil)
		require.NoError(t, err)
...
func TestInternalSessionInsertAndVerify(t *testing.T) {
	...
	for i := 1; i <= 3; i++ {
		rows, err := session.ExecutePrepared(ctx, prepared, tree.Datums{
...
func TestTransaction(t *testing.T) {
	...
	err = session.Txn(ctx, func(ctx context.Context) error {
		_, err := session.ExecutePrepared(ctx, prepared, tree.Datums{
...
func TestInternalSession(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			rows, err := session.QueryPrepared(ctx, hc, nil)
			require.NoError(t, err)
			require.Equal(t, []tree.Datums{
				{tree.NewDInt(tree.DInt(1))},
			}, rows)
...
func tcTxnReadsItsOwnWrites(
	...
	err = session.Txn(ctx, func(ctx context.Context) error {
		...
		rows, err := session.QueryPrepared(ctx, selectPrepared, tree.Datums{tree.NewDInt(tree.DInt(1))})
		if err != nil {
			return err
		}
		require.Equal(t, []tree.Datums{
...
func tcCancelQueryBlockedOnSQLLock(
	...
	lockingTxn, err := db.Begin()
	require.NoError(t, err)

	// Acquire a lock by updating the row
	_, err = lockingTxn.Exec("UPDATE defaultdb.lock_test SET val = 20 WHERE id = 1")
	require.NoError(t, err)
	...
	// Create a context for the query and cancel it in 100 ms. We don't set a
	// deadline because some code within crdb observes the deadline.
	queryCtx, cancel := context.WithCancel(ctx)
	time.AfterFunc(100*time.Millisecond, cancel)

	_, err = session.ExecutePrepared(queryCtx, prepared, nil)
	require.ErrorContains(t, err, "context canceled")

	require.NoError(t, lockingTxn.Rollback())
...
func tcRetrySerializableError(
	...
	err = session.Txn(ctx, func(ctx context.Context) error {
		executions++
		// Read account 1's balance
		rows, err := session.QueryPrepared(ctx, readPrepared, tree.Datums{tree.NewDInt(tree.DInt(1))})
		if err != nil {
			return err
		}
		...
		_, err = session.ExecutePrepared(ctx, updatePrepared, tree.Datums{
	...
	require.NoError(t, err)
	require.GreaterOrEqual(t, executions, 2, "expected at least one retry due to serializable error")
	...
	rows, err = session.QueryPrepared(ctx, readPrepared, tree.Datums{tree.NewDInt(tree.DInt(2))})
	require.NoError(t, err)
	require.Equal(t, 1, len(rows))

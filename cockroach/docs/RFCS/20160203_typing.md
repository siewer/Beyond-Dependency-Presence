...
11. `TYPEASSERT_NUMERIC(<expression>, <type>)` accepts an expression of type
    ...
    of the expression is returned, cast to the type. Otherwise, a
...
Numbers gets typed and cast as exact, multiplication
...
cast placeholders (but only placeholders!), that is, folding:
...
1. Constant folding.
   ...
   expressions.  For number literals, the implementation type from the
...
language enthusiasts, with possible benefits in terms of external
contributions.
...
Next, during type checking, ``NumVals`` will be evaluated as their
...
added using exact arithmetic in the first folding phase to

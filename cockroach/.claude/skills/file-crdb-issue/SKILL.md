...
fit the above), use the investigation structure in the Investigation
section below instead of a template.

If the issue type is obvious from context (e.g., the user described a bug),
pick the matching template without asking. Only ask when ambiguous.
...
Query the repo's labels to find the appropriate team and areas for the issue if
...
Use the label conventions table below to guide which prefixes to search.
Match labels to the issue's team and areas. Do not invent labels — only
use ones that exist in the repo.
...
For research/analysis work, don't use a bug report template. Instead,
file directly with this structure:

...
class DetermineExecPathTest : public ::testing::Test {
 protected:
    void
    SetUp() override {
        ...
        varchar_no_index_fid_ =
            schema_->AddDebugField("varchar_no_idx", DataType::VARCHAR);
    ...
    FieldId varchar_no_index_fid_;
    int64_t N_ = 1000;
    std::shared_ptr<GeneratedData> raw_data_;
};

// --------------------------------------------------------------------------
// Group 1: DetermineExecPath — No Index
// --------------------------------------------------------------------------
TEST_F(DetermineExecPathTest, DetermineExecPath_NoIndex) {
...
// --------------------------------------------------------------------------
// Group 2: DetermineExecPath — With Index
// --------------------------------------------------------------------------
TEST_F(DetermineExecPathTest, DetermineExecPath_WithIndex) {
...
// --------------------------------------------------------------------------
// Group 3: DetermineExecPath — ARRAY type never uses index
// --------------------------------------------------------------------------
class DetermineExecPathArrayTest : public ::testing::Test {
 protected:
    void
    SetUp() override {
        schema_ = std::make_shared<Schema>();
...
TEST_F(DetermineExecPathArrayTest, DetermineExecPath_ArrayType) {
    auto raw_data = DataGen(schema_, N_);
...
// --------------------------------------------------------------------------
TEST_F(DetermineExecPathTest, CanExecuteAllAtOnce_AlwaysTrueExpr) {
    auto segment = CreateSegmentNoIndex();
...
// --------------------------------------------------------------------------
TEST_F(DetermineExecPathTest, CanExecuteAllAtOnce_LogicalBinaryBothIndexed) {
    auto segment = CreateSegmentWithInt64Index();
...
TEST_F(DetermineExecPathTest, CanExecuteAllAtOnce_LogicalBinaryMixed) {
    auto segment = CreateSegmentWithInt64Index();

    // (int64 == 42) AND (varchar_no_idx < "abc")
    // int64 has index, varchar_no_idx is non-PK and has no index
    auto expr1 = MakeUnaryRangeExpr(
        int64_fid_, DataType::INT64, proto::plan::OpType::Equal, 42);
...
TEST_F(DetermineExecPathTest, CanExecuteAllAtOnce_LogicalUnary) {
    auto segment = CreateSegmentWithInt64Index();
    ...
    // NOT(varchar_no_idx < "abc") — child does NOT use index (non-PK, no index)
    proto::plan::GenericValue str_val;
    str_val.set_string_val("abc");
...
// --------------------------------------------------------------------------
TEST_F(DetermineExecPathTest, ExprSet_CanExecuteAllAtOnce) {
    ...
    // Mix: varchar_no_idx is non-PK and has no index
    proto::plan::GenericValue str_val;
    str_val.set_string_val("abc");
...
// --------------------------------------------------------------------------
TEST_F(DetermineExecPathTest, FilterBitsNode_ExecuteAllAtOnce_Correctness) {
    auto segment = CreateSegmentWithInt64Index();
...
TEST_F(DetermineExecPathTest, FilterBitsNode_NoIndex_Correctness) {
    auto segment = CreateSegmentNoIndex();

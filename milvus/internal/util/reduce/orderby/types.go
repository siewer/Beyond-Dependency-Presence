...
func ParseOrderByFields(orderByStrs []string, schema *schemapb.CollectionSchema) ([]*OrderByField, error) {
	...
	for _, orderByStr := range orderByStrs {
		...
		// Note: dynamic fields (stored in $meta JSON column) are not in schema.Fields,
...
// ConvertFromPlanOrderByFields converts planpb.OrderByField to orderby.OrderByField
// using schema to look up FieldName and DataType (which planpb doesn't carry).
func ConvertFromPlanOrderByFields(planFields []*planpb.OrderByField, schema *schemapb.CollectionSchema) ([]*OrderByField, error) {
	if len(planFields) == 0 {
		return nil, nil
	}

	fieldIDToSchema := make(map[int64]*schemapb.FieldSchema, len(schema.Fields))
	for _, field := range schema.Fields {
		fieldIDToSchema[field.FieldID] = field
	}

	result := make([]*OrderByField, len(planFields))
	for i, pf := range planFields {
		...
		result[i] = &OrderByField{
			...
			FieldName:  fs.GetName(),
			Ascending:  pf.GetAscending(),
			NullsFirst: pf.GetNullsFirst(),
			DataType:   fs.GetDataType(),
		}
	}
	return result, nil
}

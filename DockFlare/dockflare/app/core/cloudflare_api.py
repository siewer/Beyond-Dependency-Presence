...
def parse_tunnel_rules_for_migration(tunnel_config):
    ...
    for rule in ingress_rules:
        ...
        rule_data = {
            ...
            "zone_id": None,
            "no_tls_verify": False,
            "origin_server_name": rule.get("originRequest", {}).get("originServerName"),
            "http_host_header": rule.get("originRequest", {}).get("httpHostHeader"),
            "http2_origin": bool(rule.get("originRequest", {}).get("http2Origin")),
            "disable_chunked_encoding": bool(rule.get("originRequest", {}).get("disableChunkedEncoding")),
            "match_sni_to_host": bool(rule.get("originRequest", {}).get("matchSNIToHost")),
            "access_app_id": None,

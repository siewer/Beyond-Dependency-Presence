...
| `dockflare.match_sni_to_host` | `true` に設定すると、CloudflareはTLSハンドシェイク時に受信リクエストのホスト名に合わせて自動的にServer Name Indication（SNI）を設定します。 | `dockflare.match_sni_to_host=true` |

...
export type KmipApplicationModel = ModelFrom<KmipApplicationRoute>;

export default class KmipApplicationRoute extends Route {
  ...
  async model(params: Record<string, unknown>, transition: Transition) {
    ...
    let config: KmipConfigureRequest | undefined;
    let promptConfig = false;
    let configError: unknown;
    // check if engine is configured
    // child routes will handle prompting for configuration if needed
    try {
      const { data } = await this.api.secrets.kmipReadConfiguration(secretsEngine.id);
      config = data as KmipConfigureRequest;
    } catch (error) {

...
START_DATETIME = datetime.datetime(2011, 1, 1).replace(tzinfo=timezone.utc)
END_DATETIME = datetime.datetime(2012, 1, 1).replace(tzinfo=timezone.utc)
User = get_user_model()
...
class TestSavingAVoucher(TestCase):
    def test_saves_code_as_uppercase(self):
        voucher = VoucherFactory(
            code="lower", start_datetime=START_DATETIME, end_datetime=END_DATETIME
        )
    ...
    def test_verifies_dates_are_sensible(self):
        with self.assertRaises(exceptions.ValidationError):
            voucher = Voucher.objects.create(
                code="lower", start_datetime=END_DATETIME, end_datetime=START_DATETIME
            )
            voucher.clean()
...
class TestAVoucher(TestCase):
    ...
    def test_is_active_between_start_and_end_dates(self):
        test = datetime.datetime(2011, 6, 10).replace(tzinfo=timezone.utc)
    ...
    def test_is_inactive_outside_of_start_and_end_dates(self):
        test = datetime.datetime(2012, 3, 10).replace(tzinfo=timezone.utc)
...
class TestAvailableForBasket(TestCase):
    def setUp(self):
        ...
        product_range = RangeFactory(products=[self.product])
        condition = ConditionFactory(range=product_range, value=2)
        self.voucher = VoucherFactory()
        self.voucher.offers.add(
            create_offer(
                offer_type="Voucher", product_range=product_range, condition=condition
            )
        )
...
class TestVoucherSet(object):
    def test_factory(self):
        ...
        for voucher in voucherset.vouchers.all():
            ...
            assert list(voucher.offers.all()) == list(offers)
            assert voucher.offers.count() == 1
            assert (
                voucher.offers.filter(offer_type=ConditionalOffer.VOUCHER).count() == 1
            )

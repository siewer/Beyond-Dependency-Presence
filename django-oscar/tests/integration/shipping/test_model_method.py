...
class TestOrderAndItemCharges(TestCase):
    ...
    def test_single_item_basket(self):
        ...
        charge = self.method.calculate(basket)
        self.assertEqual(D("5.00") + D("1.00"), charge.incl_tax)
...
class WeightBasedMethodTests(TestCase):
    ...
    def test_zero_charge_with_shipping_discount(self):
        offer = factories.create_offer(
            benefit=Benefit.objects.create(type=Benefit.SHIPPING_FIXED_PRICE, value=1),
        )
        basket = factories.create_basket()
        ...
        method = Repository().apply_shipping_offer(basket, self.standard, offer)
        self.assertEqual(D("0.00"), method.discount(basket))

    def test_get_charge_when_weight_is_divided_by_top_band_upper_limit_without_remainder(
        self,
    ):

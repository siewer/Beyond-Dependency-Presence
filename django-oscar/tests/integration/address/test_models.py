...
from tests._site.model_tests_app.models import (
    UserAddressModelWithCustomBaseFields,
...
class TestUserAddress(TestCase):
    ...
    def test_hash_and_summary_values_on_model_with_custom_base_fields(self):
        a = UserAddressModelWithCustomBaseFieldsFactory.build(
    ...
    def test_hash_and_summary_values_on_model_with_custom_hash_fields(self):
        a = UserAddressModelWithCustomHashFieldsFactory.build(
            country=self.country, user=self.user
        )
        self.assertEqual(
            a.summary,
    ...
    def test_converts_postcode_to_uppercase_when_cleaning(self):
        address = factories.UserAddressFactory.build(
            postcode="n4 8ty", country=self.country, user=self.user
        )
        address.clean()
    ...
    def test_summary_is_property(self):
        ...
        a.clean()
        self.assertEqual(
            "Terry Barrington, 75 Smith Road, N4 8TY, UNITED KINGDOM", a.summary
        )

    def test_search_text_value(self):
        a = factories.UserAddressFactory()
        self.assertEqual(
            "Barry Barrington 1 King Road London SW1 9RE UNITED KINGDOM", a.search_text
        )

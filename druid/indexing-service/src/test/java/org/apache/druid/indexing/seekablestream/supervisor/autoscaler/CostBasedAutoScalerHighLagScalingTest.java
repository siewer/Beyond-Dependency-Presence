...
public class CostBasedAutoScalerHighLagScalingTest
{
  ...
  public void testComputeExtraPPTIncrease()
  {
    ...
    Assert.assertEquals("C=4, 200K lag should yield a modest PPT increase", 6, boost4_200k);
    ...
    Assert.assertEquals("C=12, 200K lag should not change PPT", 0, boost12_200k);
    ...
    Assert.assertEquals("C=24, 200K lag should not change PPT", 0, boost24_200k);
  }
  ...
  public void testComputeExtraPPTIncreaseInvalidInputs()
  {
    Assert.assertEquals(
        0,
        CostBasedAutoScaler.computeExtraPPTIncrease(LAG_THRESHOLD, 1_000_000, 0, 4, 48)
    );
    Assert.assertEquals(
        0,
        CostBasedAutoScaler.computeExtraPPTIncrease(LAG_THRESHOLD, 1_000_000, 48, 0, 48)
    );
    Assert.assertEquals(
        0,
        CostBasedAutoScaler.computeExtraPPTIncrease(LAG_THRESHOLD, 1_000_000, 48, 4, 0)
    );
    Assert.assertEquals(
        0,
        CostBasedAutoScaler.computeExtraPPTIncrease(LAG_THRESHOLD, 1_000_000, -1, 4, 48)
    );
  }
}

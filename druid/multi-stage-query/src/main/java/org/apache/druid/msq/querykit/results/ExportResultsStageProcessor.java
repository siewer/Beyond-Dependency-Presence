...
public class ExportResultsStageProcessor implements StageProcessor<List<String>, Object>
{
  ...
  @Override
  public ListenableFuture<List<String>> execute(ExecutionContext context)
  {
    final StandardStageRunner<Object, List<String>> stageRunner = new StandardStageRunner<>(context);
      ...
      return stageRunner.run(
          new ProcessorsAndChannels<>(
              ProcessorManagers.of(Sequences.<ExportResultsFrameProcessor>empty())
                               .withAccumulation(new ArrayList<>(), (acc, file) -> acc),
              OutputChannels.none()
          )
      );
    }
    ...
    final Sequence<FrameProcessor<Object>> processors = readableInputs.map(
        readableInput -> new ExportResultsFrameProcessor(
            ...
            exportStorageProvider.createStorageConnector(context.frameContext().tempDir()),
  ...
  @Override
  @Nullable
  public TypeReference<List<String>> getResultTypeReference()
  {
    return new TypeReference<>() {};
  }

  @Override
  public List<String> mergeAccumulatedResult(List<String> accumulated, List<String> otherAccumulated)
  {

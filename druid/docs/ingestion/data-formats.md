...
| Field | Type | Description | Required |
|-------|------|-------------|----------|
| type | String | Set value to `string` for most cases. | yes |
...
Note that `hasHeaderRow` and `skipHeaderRows` are effective only for batch index tasks. Other types of index

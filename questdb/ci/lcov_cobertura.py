...
CPPFILT = "c++filt"
HAVE_CPPFILT = shutil.which(CPPFILT) is not None
...
class LcovCobertura:
    ...
    def parse(self, **kwargs):
        ...
        for line in self.lcov_data.split("\n"):
            ...
            if input_type == "SF":
                # Get file name
                file_name = os.path.normpath(line_parts[-1].strip())
                # The path is in the format of /xxx/yyy/[base_dir]/path/to/file
                # We need to make it relative to base_dir
                if not self.base_dir in file_name:
                    print(
                        "Warning: base_dir '%s' not found in file path '%s', skipping."
                        % (self.base_dir, file_name)
                    )
                    continue
                relative_file_name = file_name.split(self.base_dir, 1)[-1].lstrip(
                    os.path.sep
                )
...
def main(argv=None):
    ...
    try:
        ...
        sys.stderr.write("Unable to convert %s to Cobertura XML: %s\n" % (args[1], e))

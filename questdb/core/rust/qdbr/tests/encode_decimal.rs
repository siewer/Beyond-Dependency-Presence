...
fn encode_and_read(
    ...
    let bytes = write_parquet_no_stats(partition);
...
fn test_encode_decimal32() {
    ...
    for encoding in &DECIMAL_ENCODINGS {
        for null_pattern in &ALL_NULL_PATTERNS {
            ...
            let batches = encode_and_read(col_type, as_bytes(&data), COUNT, *encoding);
            let arr = batches[0]
                .column(0)
                .as_any()
            ...
            for (i, exp) in expected.iter().enumerate() {
                match exp {
                    Some(v) => {
                        assert!(
                            !arr.is_null(i),
                            "Decimal32 {encoding:?}/{null_pattern:?}: expected non-null at {i}"
                        );
                        assert_eq!(
                            arr.value(i),
                            *v,
                            "Decimal32 {encoding:?}/{null_pattern:?}: mismatch at {i}"
                        );
                    }
                    None => {
                        assert!(
                            arr.is_null(i),
                            "Decimal32 {encoding:?}/{null_pattern:?}: expected null at {i}"
                        );
                    }
                }
            }
            ...
            for (i, exp) in expected.iter().enumerate() {
                match exp {
                    Some(v) => {
                        assert!(
                            !arr.is_null(i),
                            "Decimal64 {encoding:?}/{null_pattern:?}: expected non-null at {i}"
                        );
                        assert_eq!(
                            arr.value(i),
                            *v,
                            "Decimal64 {encoding:?}/{null_pattern:?}: mismatch at {i}"
                        );
                    }
                    None => {
                        assert!(
                            arr.is_null(i),
                            "Decimal64 {encoding:?}/{null_pattern:?}: expected null at {i}"
                        );
                    }
                }
            }
            ...
            for (i, exp) in expected.iter().enumerate() {
                match exp {
                    Some(v) => {
                        assert!(
                            !arr.is_null(i),
                            "Decimal8 {encoding:?}/{null_pattern:?}: expected non-null at {i}"
                        );
                        assert_eq!(
                            arr.value(i),
                            *v,
                            "Decimal8 {encoding:?}/{null_pattern:?}: mismatch at {i}"
                        );
                    }
                    None => {
                        assert!(
                            arr.is_null(i),
                            "Decimal8 {encoding:?}/{null_pattern:?}: expected null at {i}"
                        );
                    }
                }
            }
            ...
            for (i, exp) in expected.iter().enumerate() {
                match exp {
                    Some(v) => {
                        assert!(
                            !arr.is_null(i),
                            "Decimal16 {encoding:?}/{null_pattern:?}: expected non-null at {i}"
                        );
                        assert_eq!(
                            arr.value(i),
                            *v,
                            "Decimal16 {encoding:?}/{null_pattern:?}: mismatch at {i}"
                        );
                    }
                    None => {
                        assert!(
                            arr.is_null(i),
                            "Decimal16 {encoding:?}/{null_pattern:?}: expected null at {i}"
                        );
                    }
                }
            }
...
fn test_encode_decimal128() {
    ...
    for encoding in &DECIMAL_ENCODINGS {
        for null_pattern in &ALL_NULL_PATTERNS {
            ...
            let batches = encode_and_read(col_type, &data_bytes, COUNT, *encoding);
            let arr = batches[0]
                .column(0)
                .as_any()
                .downcast_ref::<Decimal128Array>()
            ...
            for (i, exp) in expected.iter().enumerate() {
                match exp {
                    Some(v) => {
                        assert!(
                            !arr.is_null(i),
                            "Decimal128 {encoding:?}/{null_pattern:?}: expected non-null at {i}"
                        );
                        assert_eq!(
                            arr.value(i),
                            *v,
                            "Decimal128 {encoding:?}/{null_pattern:?}: mismatch at {i}"
                        );
                    }
                    None => {
                        assert!(
                            arr.is_null(i),
                            "Decimal128 {encoding:?}/{null_pattern:?}: expected null at {i}"
                        );
                    }
                }
            }
            ...
            for (i, exp) in expected.iter().enumerate() {
                match exp {
                    Some(v) => {
                        assert!(
                            !arr.is_null(i),
                            "Decimal256 {encoding:?}/{null_pattern:?}: expected non-null at {i}"
                        );
                        assert_eq!(
                            arr.value(i),
                            *v,
                            "Decimal256 {encoding:?}/{null_pattern:?}: mismatch at {i}"
                        );
                    }
                    None => {
                        assert!(
                            arr.is_null(i),
                            "Decimal256 {encoding:?}/{null_pattern:?}: expected null at {i}"
                        );
                    }
                }
            }

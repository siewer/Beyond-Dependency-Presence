...
macro_rules! unpack_impl {
    ($t:ty, $bytes:literal, $bits:tt, $values:tt) => {
        #[inline]
        pub fn unpack<const NUM_BITS: usize>(input: &[u8], output: *mut $t) {
            ...
            assert!(input.len() >= NUM_BITS * $values / 8);

            let r = |output_idx: usize| {
                let start = output_idx * $bytes;
                if start + $bytes <= input.len() {
                    <$t>::from_le_bytes(input[start..start + $bytes].try_into().unwrap())
                } else {
                    // The final read can straddle the packed buffer boundary for
                    // low bit widths (e.g. 20-byte buffers for 5-bit u64 packs).
                    // Treat bytes beyond the buffer as zero.
                    let mut tmp = [0u8; $bytes];
                    if start < input.len() {
                        let remaining = input.len() - start;
                        tmp[..remaining].copy_from_slice(&input[start..]);
                    }
                    <$t>::from_le_bytes(tmp)
                }
            };

            seq_macro::seq!(i in 0..$values {
                ...
                if start_byte != end_byte && end_bit_offset != 0 {
                    ...
                    unsafe {
                        *output.add(i) = a | (b & mask);
                    }
                } else {
                    let val = r(start_byte);
                    unsafe {
                        *output.add(i) = (val >> start_bit_offset) & mask;
                    }
                }
            });
        }
...
macro_rules! unpack {
    ($name:ident, $t:ty, $bytes:literal, $bits:tt, $values:tt) => {
        mod $name {
            unpack_impl!($t, $bytes, $bits, $values);
        }

        /// Unpack packed `input` into `output` with a bit width of `num_bits`
        #[inline]
        pub fn $name(input: &[u8], output: *mut $t, num_bits: usize) {
...
unpack!(unpack32, u32, 4, 32, 32);
unpack!(unpack64, u64, 8, 64, 32);

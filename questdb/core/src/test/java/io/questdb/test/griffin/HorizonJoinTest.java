...
public class HorizonJoinTest extends AbstractCairoTest {
    ...
    @Test
    public void testHorizonJoinBothTablesEmptyNotKeyed() throws Exception {
        assertMemoryLeak(() -> {
            executeWithRewriteTimestamp("CREATE TABLE trades (ts #TIMESTAMP, sym SYMBOL, qty DOUBLE) TIMESTAMP(ts)", leftTableTimestampType.getTypeName());
            executeWithRewriteTimestamp("CREATE TABLE prices (ts #TIMESTAMP, sym SYMBOL, price DOUBLE) TIMESTAMP(ts)", rightTableTimestampType.getTypeName());
    ...
    @Test
    public void testHorizonJoinBothTablesEmptyWithList() throws Exception {
        assertMemoryLeak(() -> {
            executeWithRewriteTimestamp("CREATE TABLE trades (ts #TIMESTAMP, sym SYMBOL, qty DOUBLE) TIMESTAMP(ts)", leftTableTimestampType.getTypeName());
            executeWithRewriteTimestamp("CREATE TABLE prices (ts #TIMESTAMP, sym SYMBOL, price DOUBLE) TIMESTAMP(ts)", rightTableTimestampType.getTypeName());
    ...
    @Test
    public void testHorizonJoinConstantWhereFalse() throws Exception {
        // WHERE FALSE should produce an empty result set, even with HORIZON JOIN.
        assertMemoryLeak(() -> {
            executeWithRewriteTimestamp("CREATE TABLE trades (ts #TIMESTAMP, sym SYMBOL, qty DOUBLE) TIMESTAMP(ts)", leftTableTimestampType.getTypeName());
            executeWithRewriteTimestamp("CREATE TABLE prices (ts #TIMESTAMP, sym SYMBOL, price DOUBLE) TIMESTAMP(ts)", rightTableTimestampType.getTypeName());
    ...
    @Test
    public void testHorizonJoinEmptyMaster() throws Exception {
        assertMemoryLeak(() -> {
            executeWithRewriteTimestamp("CREATE TABLE trades (ts #TIMESTAMP, sym SYMBOL, qty DOUBLE) TIMESTAMP(ts)", leftTableTimestampType.getTypeName());
            executeWithRewriteTimestamp("CREATE TABLE prices (ts #TIMESTAMP, sym SYMBOL, price DOUBLE) TIMESTAMP(ts)", rightTableTimestampType.getTypeName());

            // Slave has data, but master is empty
            execute(
                    """
                            INSERT INTO prices VALUES
    ...
    @Test
    public void testHorizonJoinEmptyMasterNotKeyed() throws Exception {
        assertMemoryLeak(() -> {
            executeWithRewriteTimestamp("CREATE TABLE trades (ts #TIMESTAMP, sym SYMBOL, qty DOUBLE) TIMESTAMP(ts)", leftTableTimestampType.getTypeName());
            executeWithRewriteTimestamp("CREATE TABLE prices (ts #TIMESTAMP, sym SYMBOL, price DOUBLE) TIMESTAMP(ts)", rightTableTimestampType.getTypeName());
    ...
    @Test
    public void testHorizonJoinEmptyMasterWithFilter() throws Exception {
        assertMemoryLeak(() -> {
            executeWithRewriteTimestamp("CREATE TABLE trades (ts #TIMESTAMP, sym SYMBOL, qty DOUBLE) TIMESTAMP(ts)", leftTableTimestampType.getTypeName());
            executeWithRewriteTimestamp("CREATE TABLE prices (ts #TIMESTAMP, sym SYMBOL, price DOUBLE) TIMESTAMP(ts)", rightTableTimestampType.getTypeName());
            ...
            // Filter eliminates all master rows
            String sql = "SELECT h.offset / " + getSecondsDivisor() + " AS sec_offs, avg(p.price) " +
                    "FROM trades AS t " +
                    "HORIZON JOIN prices AS p ON (t.sym = p.sym) " +
    ...
    @Test
    public void testHorizonJoinEmptySlave() throws Exception {
        assertMemoryLeak(() -> {
            executeWithRewriteTimestamp("CREATE TABLE trades (ts #TIMESTAMP, sym SYMBOL, qty DOUBLE) TIMESTAMP(ts)", leftTableTimestampType.getTypeName());
            executeWithRewriteTimestamp("CREATE TABLE prices (ts #TIMESTAMP, sym SYMBOL, price DOUBLE) TIMESTAMP(ts)", rightTableTimestampType.getTypeName());

            // Master has data, but slave is empty
            execute(
                    """
                            INSERT INTO trades VALUES
                                ('1970-01-01T00:00:01.000000Z', 'AX', 100),
                                ('1970-01-01T00:00:02.000000Z', 'BX', 200)
                            """
            );
    ...
    @Test
    public void testHorizonJoinEmptySlaveNotKeyed() throws Exception {
        assertMemoryLeak(() -> {
            executeWithRewriteTimestamp("CREATE TABLE trades (ts #TIMESTAMP, sym SYMBOL, qty DOUBLE) TIMESTAMP(ts)", leftTableTimestampType.getTypeName());
            executeWithRewriteTimestamp("CREATE TABLE prices (ts #TIMESTAMP, sym SYMBOL, price DOUBLE) TIMESTAMP(ts)", rightTableTimestampType.getTypeName());
    ...
    @Test
    public void testHorizonJoinEmptySlaveWithArrayColumn() throws Exception {
        // Reproduces NPE in DoubleArrayAccessFunctionFactory when the slave
        // record is null (no ASOF match) and the query accesses array elements.
        assertMemoryLeak(() -> {
            executeWithRewriteTimestamp(
            ...
            executeWithRewriteTimestamp(
                    "CREATE TABLE prices (ts #TIMESTAMP, sym SYMBOL, arr DOUBLE[]) TIMESTAMP(ts)",
                    rightTableTimestampType.getTypeName()
            );
            ...
            // Slave table is empty, so ASOF JOIN will not find a match.
            // Accessing arr[1] on a null slave record should return NaN, not NPE.
            String sql = "SELECT h.offset / " + getSecondsDivisor() + " AS sec_offs, avg(p.arr[1]), sum(t.qty) " +
                    "FROM trades AS t " +
                    "HORIZON JOIN prices AS p ON (t.sym = p.sym) " +
                    "RANGE FROM 0s TO 1s STEP 1s AS h " +
                    "ORDER BY sec_offs";
    ...
    @Test
    public void testHorizonJoinRangeFromGreaterThanTo() throws Exception {
        assertMemoryLeak(() -> {
            executeWithRewriteTimestamp("CREATE TABLE trades (ts #TIMESTAMP, sym SYMBOL, price DOUBLE) TIMESTAMP(ts)", leftTableTimestampType.getTypeName());
            executeWithRewriteTimestamp("CREATE TABLE prices (ts #TIMESTAMP, sym SYMBOL, price DOUBLE) TIMESTAMP(ts)", rightTableTimestampType.getTypeName());
    ...
    @Test
    public void testHorizonJoinWithExplicitGroupByColumnPrefixAsymmetry() throws Exception {
        // Test that GROUP BY without table prefix matches SELECT with table prefix and vice versa
        assertMemoryLeak(() -> {
            executeWithRewriteTimestamp("CREATE TABLE trades (ts #TIMESTAMP, sym SYMBOL, qty DOUBLE) TIMESTAMP(ts)", leftTableTimestampType.getTypeName());
            executeWithRewriteTimestamp("CREATE TABLE prices (ts #TIMESTAMP, sym SYMBOL, price DOUBLE) TIMESTAMP(ts)", rightTableTimestampType.getTypeName());
            ...
            // SELECT uses t.sym (prefixed) but GROUP BY uses sym (unprefixed)
            String sql = "SELECT h.offset / " + getSecondsDivisor() + " AS sec_offs, t.sym, avg(p.price) " +
                    "FROM trades AS t " +
                    "HORIZON JOIN prices AS p ON (t.sym = p.sym) " +
                    "RANGE FROM 0s TO 0s STEP 1s AS h " +

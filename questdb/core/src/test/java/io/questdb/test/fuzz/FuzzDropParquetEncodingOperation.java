...
public class FuzzDropParquetEncodingOperation implements FuzzTransactionOperation {
    private final String columnName;

    public FuzzDropParquetEncodingOperation(String columnName) {
    ...
    public boolean apply(Rnd tempRnd, CairoEngine engine, TableWriterAPI wApi, int virtualTimestampIndex, LongList excludedTsIntervals) {
        try (SqlExecutionContextImpl context = new SqlExecutionContextImpl(engine, 1);
            ...
            String sql = "ALTER TABLE " + wApi.getTableToken().getTableName() +
                    " ALTER COLUMN " + columnName + " DROP PARQUET";

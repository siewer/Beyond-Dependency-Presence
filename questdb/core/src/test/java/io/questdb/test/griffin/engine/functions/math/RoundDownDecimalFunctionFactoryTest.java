...
import io.questdb.griffin.engine.functions.math.RoundDownDecimalFunctionFactory;
 ...
 */
public class RoundDownDecimalFunctionFactoryTest extends AbstractCairoTest {
    private final ObjList<Function> args = new ObjList<>();
    private final RoundDownDecimalFunctionFactory factory = new RoundDownDecimalFunctionFactory();
    ...
    public void testRoundDownPositive() {
        ...
        try (Function func = factory.newInstance(-1, args, null, configuration, sqlExecutionContext)) {
            ...
            Assert.assertEquals(9999, value);
            Assert.assertEquals(ColumnType.getDecimalType(5, 1), func.getType());
        }
    }
}
...
public class GeoByteColumnTest {

    @Test
    public void testNewInstanceReturnsCorrectTypeForCachedColumnIndexes() {
        for (int col = 0; col < 40; col++) {
            assertBits(ColumnType.GEOBYTE_MIN_BITS, col);
            assertBits(ColumnType.GEOBYTE_MAX_BITS, col);
        }
    }

    private static void assertBits(int bits, int column) {
        int type = ColumnType.getGeoHashTypeWithBits(bits);
        GeoByteColumn col = GeoByteColumn.newInstance(column, type);
        ...
        boolean isCached = GeoByteColumn.newInstance(column, type) == GeoByteColumn.newInstance(column, type);

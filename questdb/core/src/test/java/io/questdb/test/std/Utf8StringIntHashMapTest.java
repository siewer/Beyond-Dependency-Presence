...
public class Utf8StringIntHashMapTest {
    ...
    public void testHashMapCompatibility() {
        ...
        Utf8StringIntHashMap ourMap = new Utf8StringIntHashMap();
        try {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (v != null) {
                    ...
                    Assert.assertEquals(-1, ourMap.get(dus));
                }
                p += len;
            }
        } finally {
    ...
    public void testHashMapUtf8() {
        ...
        long mem = Unsafe.malloc(memSize, MemoryTag.NATIVE_DEFAULT);
        final DirectUtf8String dus = new DirectUtf8String();
        Utf8StringIntHashMap map = new Utf8StringIntHashMap();
        try {
            ...
            for (int i = 0; i < N; i++) {
                ...
                Assert.assertEquals(-1, map.get(dus));

                final Utf8String us = Utf8String.newInstance(dus);
                map.put(Utf8String.newInstance(dus), i);
                Assert.assertEquals(i, map.get(dus));
                Assert.assertEquals(i, map.get(us));
            }

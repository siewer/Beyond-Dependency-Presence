...
public abstract class AbstractLowerCaseUtf8SequenceHashSet implements Mutable {
    ...
    public int keyIndex(Utf8Sequence key, int lo, int hi) {
        int hashCode = Utf8s.lowerCaseAsciiHashCode(key, lo, hi);
        int index = Hash.spread(hashCode) & mask;
        if (keys[index] == noEntryKey) {
            return index;
        }
        Utf8Sequence us = keys[index];
        if (hashCode == hashCodes[index] && Utf8s.equalsIgnoreCaseAscii(key, lo, hi, us, 0, us.size())) {
            return -index - 1;
        }
    ...
    private int probe(Utf8Sequence key, int hashCode, int lo, int hi, int index) {
        do {
            ...
            if (hashCode == hashCodes[index] && Utf8s.equalsIgnoreCaseAscii(key, lo, hi, cs, 0, cs.size())) {
                return -index - 1;
            }
        } while (true);
    }

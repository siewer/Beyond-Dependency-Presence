...
class AvgDecimal256Rescale256GroupByFunction extends Decimal256Function implements GroupByFunction, UnaryFunction {
    ...
    public AvgDecimal256Rescale256GroupByFunction(@NotNull Function arg, int position, int targetType) {
        super(targetType);
        this.arg = arg;
    ...
    private boolean calc(Record rec) {
        ...
        if (count > 0) {
            ...
            try {
                ...
                final int targetScale = ColumnType.getDecimalScale(type);
                decimal256A.divide(0, 0, 0, count, 0, targetScale, RoundingMode.HALF_EVEN);
            } catch (NumericException e) {
                throw CairoException.nonCritical().position(position).put("avg aggregation failed: ").put(e.getFlyweightMessage());
            }
            return true;

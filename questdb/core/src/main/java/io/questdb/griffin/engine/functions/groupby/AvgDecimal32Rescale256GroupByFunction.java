...
/**
 * Computes AVG for DECIMAL32 inputs. We accumulate into a {@link Decimal128} to avoid overflowing a
 * long while keeping the per-row cost low. When the result is materialised the rescale step can
 * demand more precision than 128 bits, so the accumulated value is finally expressed as a
 * {@link Decimal256}.
 */
class AvgDecimal32Rescale256GroupByFunction extends Decimal256Function implements GroupByFunction, UnaryFunction {
    ...
    public AvgDecimal32Rescale256GroupByFunction(@NotNull Function arg, int position, int targetType) {
        super(targetType);
        this.arg = arg;

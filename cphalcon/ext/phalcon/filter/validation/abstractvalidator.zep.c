...
PHP_METHOD(Phalcon_Filter_Validation_AbstractValidator, getOption)
{
	...
	_1 = ZEPHIR_IS_STRING_IDENTICAL(&key, "attribute");
	if (_1) {
		_1 = Z_TYPE_P(&value) == IS_ARRAY;
	}
	if (_1) {
		zephir_memory_observe(&fieldValue);
		if (zephir_array_isset_fetch(&fieldValue, &value, &key, 0)) {
...
PHP_METHOD(Phalcon_Filter_Validation_AbstractValidator, getTemplate)
{
	...
	if (_0) {
		...
		zephir_array_fetch(&_3$$3, &_2$$3, &field, PH_NOISY | PH_READONLY, "phalcon/Filter/Validation/AbstractValidator.zep", 111);
		RETURN_CTOR(&_3$$3);
	}
...
PHP_METHOD(Phalcon_Filter_Validation_AbstractValidator, messageFactory)
{
	...
	if (Z_TYPE_P(field) == IS_ARRAY) {
		...
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_filter_validation_exception_ce, "The field can not be printed", "phalcon/Filter/Validation/AbstractValidator.zep", 166);
		return;
	}
	ZEPHIR_INIT_VAR(&_0);
	ZEPHIR_INIT_VAR(&_1);
...
PHP_METHOD(Phalcon_Filter_Validation_AbstractValidator, setTemplates)
{
	...
	zephir_is_iterable(&templates, 0, "phalcon/Filter/Validation/AbstractValidator.zep", 226);
...
PHP_METHOD(Phalcon_Filter_Validation_AbstractValidator, allowEmpty)
{
	...
	if (Z_TYPE_P(&allowEmpty) == IS_ARRAY) {
		...
		if (zephir_array_isset(&allowEmpty, field)) {
			...
			zephir_array_fetch(&_2$$3, &allowEmpty, field, PH_NOISY, "phalcon/Filter/Validation/AbstractValidator.zep", 257);
		} else {
...
PHP_METHOD(Phalcon_Filter_Validation_AbstractValidator, checkArray)
{
	...
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	zval field;
	zval *value = NULL, value_sub, *field_param = NULL, _1$$3;
	...
	ZEPHIR_METHOD_GLOBALS_PTR = pecalloc(1, sizeof(zephir_method_globals), 0);
	zephir_memory_grow_stack(ZEPHIR_METHOD_GLOBALS_PTR, __func__);
	zephir_fetch_params(1, 2, 0, &value, &field_param);
	ZEPHIR_SEPARATE_PARAM(value);
	zephir_get_strval(&field, field_param);
	_0 = Z_TYPE_P(value) == IS_ARRAY;
	if (_0) {
		_0 = zephir_array_isset(value, &field);
	}
	if (_0) {
		zephir_array_fetch(&_1$$3, value, &field, PH_NOISY | PH_READONLY, "phalcon/Filter/Validation/AbstractValidator.zep", 277);
...
PHP_METHOD(Phalcon_Filter_Validation_AbstractValidator, prepareCode)
{
	...
	if (Z_TYPE_P(&code) == IS_ARRAY) {
		zephir_array_fetch(&_2$$3, &code, &field, PH_NOISY | PH_READONLY, "phalcon/Filter/Validation/AbstractValidator.zep", 297);
...
PHP_METHOD(Phalcon_Filter_Validation_AbstractValidator, prepareLabel)
{
	...
	if (Z_TYPE_P(&label) == IS_ARRAY) {
		zephir_array_fetch(&_1$$3, &label, &field, PH_NOISY | PH_READONLY, "phalcon/Filter/Validation/AbstractValidator.zep", 318);

...
PHP_METHOD(Phalcon_DataMapper_Query_Select, appendJoin)
{
	...
	zephir_array_fetch_string(&_9, &_8, SL("FROM"), PH_NOISY | PH_READONLY, "phalcon/DataMapper/Query/Select.zep", 161);
	zephir_array_fetch(&_10, &_9, &end, PH_NOISY | PH_READONLY, "phalcon/DataMapper/Query/Select.zep", 161);
	zephir_array_fetch(&_11, &_10, &key, PH_NOISY | PH_READONLY, "phalcon/DataMapper/Query/Select.zep", 161);
	ZEPHIR_INIT_VAR(&_12);
...
PHP_METHOD(Phalcon_DataMapper_Query_Select, columns)
{
	...
	if (Z_TYPE_P(&columns) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&columns), _2, _3, _0)
		{
			...
			if (Z_TYPE_P(&key) == IS_LONG) {
				...
				ZEPHIR_CONCAT_VSV(&_4$$5, &value, " AS ", &key);
				zephir_array_append(&localColumns, &_4$$5, PH_SEPARATE, "phalcon/DataMapper/Query/Select.zep", 183);
			}
		...
		while (1) {
			...
			ZEPHIR_CALL_METHOD(&key, &columns, "key", NULL, 0);
			zephir_check_call_status();
			...
			zephir_check_call_status();
				if (Z_TYPE_P(&key) == IS_LONG) {
					...
					ZEPHIR_CONCAT_VSV(&_5$$8, &value, " AS ", &key);
					zephir_array_append(&localColumns, &_5$$8, PH_SEPARATE, "phalcon/DataMapper/Query/Select.zep", 183);
				}
	...
	zephir_array_fetch_string(&_8, &_7, SL("COLUMNS"), PH_NOISY | PH_READONLY, "phalcon/DataMapper/Query/Select.zep", 188);
	zephir_fast_array_merge(&_6, &_8, &localColumns);
	ZEPHIR_INIT_VAR(&_9);
	ZVAL_STRING(&_9, "COLUMNS");
	zephir_update_property_array(this_ptr, SL("store"), &_9, &_6);
	RETURN_THIS();
}
...
PHP_METHOD(Phalcon_DataMapper_Query_Select, getStatement)
{
	...
	zephir_array_fetch_string(&_2, &_1, SL("UNION"), PH_NOISY | PH_READONLY, "phalcon/DataMapper/Query/Select.zep", 242);
...
PHP_METHOD(Phalcon_DataMapper_Query_Select, hasColumns)
{
	...
	zephir_array_fetch_string(&_1, &_0, SL("COLUMNS"), PH_NOISY | PH_READONLY, "phalcon/DataMapper/Query/Select.zep", 266);
...
PHP_METHOD(Phalcon_DataMapper_Query_Select, join)
{
	...
	zephir_array_fetch_string(&_20, &_19, SL("FROM"), PH_NOISY | PH_READONLY, "phalcon/DataMapper/Query/Select.zep", 327);
	ZEPHIR_CALL_FUNCTION(&key, "array_key_last", NULL, 40, &_20);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_21);
...
PHP_METHOD(Phalcon_DataMapper_Query_Select, reset)
{
	...
	ZEPHIR_CALL_PARENT(NULL, phalcon_datamapper_query_select_ce, getThis(), "reset", NULL, 0);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_0);
	ZEPHIR_INIT_NVAR(&_0);
	ZVAL_STRING(&_0, "");
...
PHP_METHOD(Phalcon_DataMapper_Query_Select, buildColumns)
{
	...
	if (!(zephir_is_true(&_0))) {
		...
		zephir_array_fetch_string(&columns, &_2$$4, SL("COLUMNS"), PH_NOISY, "phalcon/DataMapper/Query/Select.zep", 449);
	}
	ZEPHIR_INIT_VAR(&_3);
	ZVAL_STRING(&_3, ",");
...
PHP_METHOD(Phalcon_DataMapper_Query_Select, buildFrom)
{
	...
	array_init(&from);
	zephir_read_property(&_0, this_ptr, ZEND_STRL("store"), PH_NOISY_CC | PH_READONLY);
	zephir_array_fetch_string(&_1, &_0, SL("FROM"), PH_NOISY | PH_READONLY, "phalcon/DataMapper/Query/Select.zep", 465);
	if (ZEPHIR_IS_EMPTY(&_1)) {
		RETURN_MM_STRING("");
	}
	zephir_read_property(&_2, this_ptr, ZEND_STRL("store"), PH_NOISY_CC | PH_READONLY);
	zephir_array_fetch_string(&_3, &_2, SL("FROM"), PH_NOISY | PH_READONLY, "phalcon/DataMapper/Query/Select.zep", 469);
	zephir_is_iterable(&_3, 0, "phalcon/DataMapper/Query/Select.zep", 473);
	if (Z_TYPE_P(&_3) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_3), _4)
		{
			...
			zephir_array_append(&from, &_10$$4, PH_SEPARATE, "phalcon/DataMapper/Query/Select.zep", 470);
		} ZEND_HASH_FOREACH_END();
	} else {
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				zephir_array_append(&from, &_13$$5, PH_SEPARATE, "phalcon/DataMapper/Query/Select.zep", 470);

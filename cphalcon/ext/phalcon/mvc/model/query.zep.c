...
PHP_METHOD(Phalcon_Mvc_Model_Query, __construct)
{
	...
	zval options;
	zval *phql_param = NULL, *container = NULL, container_sub, *options_param = NULL, __$true, __$false, __$null, enableImplicitJoins, _0, _1;
	zval phql;
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("phql"), &phql);
	...
	ZEPHIR_INIT_VAR(&_0);
	array_init(&_0);
	zephir_update_property_zval(this_ptr, ZEND_STRL("bindParams"), &_0);
	ZEPHIR_INIT_VAR(&_1);
	array_init(&_1);
	zephir_update_property_zval(this_ptr, ZEND_STRL("bindTypes"), &_1);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Mvc_Model_Query, execute)
{
	...
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *bindParams_param = NULL, *bindTypes_param = NULL, adapter, cache, cacheLifetime, cacheOptions, cacheService, defaultBindParams, defaultBindTypes, intermediate, key, lifetime, mergedParams, mergedTypes, preparedResult, result, type, uniqueRow, _0, _1$$3, _2$$3, _3$$3, _6$$3, _4$$7, _7$$10, _8$$20, _9$$20;
	...
	if (Z_TYPE_P(&cacheOptions) != IS_NULL) {
		...
		ZVAL_BOOL(&result, 0);
		ZEPHIR_CALL_METHOD(&_6$$3, &cache, "has", NULL, 0, &key);
		zephir_check_call_status();
		...
		if (!(ZEPHIR_IS_EMPTY(&result))) {
			if (UNEXPECTED(Z_TYPE_P(&result) != IS_OBJECT)) {
				ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "Cache didn't return a valid resultset", "phalcon/Mvc/Model/Query.zep", 319);
				return;
			}
			ZVAL_BOOL(&_7$$10, 0);
			ZEPHIR_CALL_METHOD(NULL, &result, "setisfresh", NULL, 0, &_7$$10);
			zephir_check_call_status();
	...
	do {
		...
		ZEPHIR_CONCAT_SV(&_9$$20, "Unknown statement ", &type);
		ZEPHIR_CALL_METHOD(NULL, &_8$$20, "__construct", NULL, 33, &_9$$20);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_8$$20, "phalcon/Mvc/Model/Query.zep", 402);
		ZEPHIR_MM_RESTORE();
		return;
	} while(0);
	...
	if (Z_TYPE_P(&cacheOptions) != IS_NULL) {
		if (!ZEPHIR_IS_LONG(&type, 309)) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "Only PHQL statements that return resultsets can be cached", "phalcon/Mvc/Model/Query.zep", 415);
			return;
		}
...
PHP_METHOD(Phalcon_Mvc_Model_Query, executeDelete)
{
	...
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *intermediate_param = NULL, *bindParams_param = NULL, *bindTypes_param = NULL, models, modelName, model, records, connection, record, exception, _0, _3, _11, _1$$4, _2$$5, _5$$7, _7$$8, _9$$6;
	...
	if (!(zephir_array_isset_fetch(&model, &_0, &modelName, 0))) {
		zephir_read_property(&_1$$4, this_ptr, ZEND_STRL("manager"), PH_NOISY_CC | PH_READONLY);
	...
	while (1) {
		...
		/* try_start_1: */
			...
			ZEPHIR_CALL_METHOD(&_5$$7, &record, "delete", NULL, 0);
			zephir_check_call_status_or_jump(try_end_1);
		...
		if (EG(exception)) {
			...
			ZVAL_OBJ(&_9$$6, EG(exception));
			Z_ADDREF_P(&_9$$6);
			if (zephir_is_instance_of(&_9$$6, SL("PDOException"))) {
	...
	if (Z_TYPE_P(&values) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&values), _4, _5, _2)
		{
			...
			do {
				...
				zephir_check_call_status();
				ZEPHIR_CALL_METHOD(NULL, &insertValue, "__construct", &_17, 36, &_15$$13);
				zephir_check_call_status();
				break;
			} while(0);
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				do {
					...
					zephir_check_call_status();
					ZEPHIR_CALL_METHOD(NULL, &insertValue, "__construct", &_17, 36, &_29$$22);
					zephir_check_call_status();
					break;
				} while(0);
...
PHP_METHOD(Phalcon_Mvc_Model_Query, executeSelect)
{
	...
	if (Z_TYPE_P(&models) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&models), _1)
		{
			...
			zephir_read_property(&_3$$3, this_ptr, ZEND_STRL("modelsInstances"), PH_NOISY_CC | PH_READONLY);
		...
		while (1) {
			...
			zephir_check_call_status();
				ZEPHIR_OBS_NVAR(&model);
				zephir_read_property(&_7$$7, this_ptr, ZEND_STRL("modelsInstances"), PH_NOISY_CC | PH_READONLY);
	...
	zephir_read_property(&_73, this_ptr, ZEND_STRL("cache"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Mvc_Model_Query, executeUpdate)
{
	...
	if (Z_TYPE_P(&fields) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&fields), _4, _5, _2)
		{
			...
			do {
				...
				ZEPHIR_CALL_METHOD(&_15$$13, &dialect, "getsqlexpression", &_16, 0, &exprValue);
				zephir_check_call_status();
				ZEPHIR_CALL_METHOD(NULL, &updateValue, "__construct", &_17, 36, &_15$$13);
				zephir_check_call_status();
				break;
			} while(0);
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				do {
					...
					zephir_check_call_status();
					ZEPHIR_CALL_METHOD(NULL, &updateValue, "__construct", &_17, 36, &_26$$22);
					zephir_check_call_status();
					break;
				} while(0);
	...
	while (1) {
		...
		/* try_start_1: */
			...
			zephir_check_call_status_or_jump(try_end_1);
			ZEPHIR_CALL_METHOD(&_31$$25, &record, "update", NULL, 0);
			zephir_check_call_status_or_jump(try_end_1);
			if (!(zephir_is_true(&_31$$25))) {
		...
		if (EG(exception)) {
			...
			Z_ADDREF_P(&_35$$24);
			if (zephir_is_instance_of(&_35$$24, SL("PDOException"))) {
				zend_clear_exception();
...
PHP_METHOD(Phalcon_Mvc_Model_Query, getQualified)
{
	...
	if (zephir_array_isset_string_fetch(&columnDomain, &expr, SL("domain"), 0)) {
		...
		if (ZEPHIR_GLOBAL(orm).column_renaming) {
			zephir_read_property(&_11$$8, this_ptr, ZEND_STRL("sqlAliasesModelsInstances"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Mvc_Model_Query, getReadConnection)
{
	...
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval intermediate, bindParams, bindTypes;
	zval *model, model_sub, *intermediate_param = NULL, *bindParams_param = NULL, *bindTypes_param = NULL, connection, transaction, _0;
	zval *this_ptr = getThis();
	...
	if (ZEPHIR_IS_LONG(&columnType, 352)) {
		...
		if (Z_TYPE_P(&_0$$4) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_0$$4), _3$$4, _4$$4, _1$$4)
			{
				...
				ZEPHIR_CALL_FUNCTION(&_5$$5, "lcfirst", &_6, 76, &modelName);
				zephir_check_call_status();
			...
			while (1) {
				...
				zephir_check_call_status();
					...
					ZEPHIR_CALL_FUNCTION(&_9$$7, "lcfirst", &_6, 76, &modelName);
					zephir_check_call_status();
...
{
	zval _13$$24, _25$$28, _46$$43, _58$$47, _90$$66, _93$$73, _99$$81, _102$$88;
	...
	if (!(merge)) {
		...
		zephir_read_property(&_70$$52, this_ptr, ZEND_STRL("models"), PH_NOISY_CC | PH_READONLY);
		zephir_fast_array_merge(&_69$$52, &_70$$52, &models);
		zephir_update_property_zval(this_ptr, ZEND_STRL("models"), &_69$$52);
		ZEPHIR_INIT_VAR(&_71$$52);
		zephir_read_property(&_72$$52, this_ptr, ZEND_STRL("modelsInstances"), PH_NOISY_CC | PH_READONLY);
		zephir_fast_array_merge(&_71$$52, &_72$$52, &modelsInstances);
		zephir_update_property_zval(this_ptr, ZEND_STRL("modelsInstances"), &_71$$52);
		ZEPHIR_INIT_VAR(&_73$$52);
		zephir_read_property(&_74$$52, this_ptr, ZEND_STRL("sqlAliases"), PH_NOISY_CC | PH_READONLY);
		zephir_fast_array_merge(&_73$$52, &_74$$52, &sqlAliases);
		zephir_update_property_zval(this_ptr, ZEND_STRL("sqlAliases"), &_73$$52);
		ZEPHIR_INIT_VAR(&_75$$52);
		zephir_read_property(&_76$$52, this_ptr, ZEND_STRL("sqlAliasesModels"), PH_NOISY_CC | PH_READONLY);
		zephir_fast_array_merge(&_75$$52, &_76$$52, &sqlAliasesModels);
		zephir_update_property_zval(this_ptr, ZEND_STRL("sqlAliasesModels"), &_75$$52);
		ZEPHIR_INIT_VAR(&_77$$52);
		zephir_read_property(&_78$$52, this_ptr, ZEND_STRL("sqlModelsAliases"), PH_NOISY_CC | PH_READONLY);
		zephir_fast_array_merge(&_77$$52, &_78$$52, &sqlModelsAliases);
		zephir_update_property_zval(this_ptr, ZEND_STRL("sqlModelsAliases"), &_77$$52);
		ZEPHIR_INIT_VAR(&_79$$52);
		zephir_read_property(&_80$$52, this_ptr, ZEND_STRL("sqlAliasesModelsInstances"), PH_NOISY_CC | PH_READONLY);
		zephir_fast_array_merge(&_79$$52, &_80$$52, &sqlAliasesModelsInstances);
		zephir_update_property_zval(this_ptr, ZEND_STRL("sqlAliasesModelsInstances"), &_79$$52);
	}
	zephir_memory_observe(&joins);
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (Z_TYPE_P(&_94$$75) == IS_ARRAY) {
					...
					ZEPHIR_CALL_METHOD(NULL, &_94$$75, "rewind", NULL, 0);
					zephir_check_call_status();
					while (1) {
						ZEPHIR_CALL_METHOD(&_96$$75, &_94$$75, "valid", NULL, 0);
						zephir_check_call_status();
						if (!zend_is_true(&_96$$75)) {
							break;
						}
						ZEPHIR_CALL_METHOD(&sqlColumn, &_94$$75, "current", NULL, 0);
						zephir_check_call_status();
							ZEPHIR_OBS_NVAR(&alias);
							if (zephir_array_isset_string_fetch(&alias, &column, SL("alias"), 0)) {
								zephir_array_update_string(&sqlColumn, SL("balias"), &alias, PH_COPY | PH_SEPARATE);
							...
							position++;
						ZEPHIR_CALL_METHOD(NULL, &_94$$75, "next", NULL, 0);
						zephir_check_call_status();
					}
				}
				ZEPHIR_INIT_NVAR(&sqlColumn);
	...
	ZEPHIR_INIT_NVAR(&column);
	zephir_memory_observe(&_103);
	zephir_read_property(&_103, this_ptr, ZEND_STRL("nestingLevel"), PH_NOISY_CC);
	zephir_update_property_array(this_ptr, SL("sqlColumnAliases"), &_103, &sqlColumnAliases);

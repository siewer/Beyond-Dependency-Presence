 ...
 * and onComplete() being called (Actor 2) in {@link CompletionStageOrderedSubscriber}.
 * <p>
 * The ordered subscriber has a different whenNextFinished path that calls
 * emptyInFlightQueueIfWeCan() under lock, exercising a different code path
...
@State
public class CompletionStageOrderedSubscriber_onComplete_JCStress {
    ...
    private final CompletionStageOrderedSubscriber<Integer, String> subscriber;

    public CompletionStageOrderedSubscriber_onComplete_JCStress() {
        ...
        subscriber = new CompletionStageOrderedSubscriber<>(mapper, downstream);
